...
- **Unique techs (full DE, not stand-ins):** Yasama ``rdg_seq_*`` (+2 arrows); Kataparuto fire rate + ``unpack_time -75%``; Crenellations range + ``passenger_attack_types infantry``; Chieftains cavalry/camel bonus + ``kill_gold_vs``; Paper Money ``gather_byproduct wood 0.014``; Circumnavigation ``reveal_map`` + ships ``time_cost -33%``; Arquebus ``projectile_lead`` + proj speed +0.5/+0.2.
