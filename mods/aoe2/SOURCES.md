...
- **Accuracy (DE Accuracy Percent):** ``rdg_cover`` / ``mdg_cover`` as 0–100 (omit or 0 = 100%%). Examples: archer 80, crossbow 85, arbalester 90, skirmisher 90, cavalry archer 50, hand cannoneer 65, longbowman 70 / elite 80, chu ko nu 85, mangudai 95, trebuchet 15 vs units / 80 vs buildings (``rdg_cover 15`` + ``rdg_cover_vs building 65``), bombard cannon 92, cannon galleon 50. Thumb Ring: ``effect bonus rdg_cover 100`` on ``archer_unit -hand_cannoneer`` (clamped to 100%%). Warwolf: same ``rdg_cover 100`` on trebuchet (units 15→100; buildings clamp 80→100) plus 0.5 blast.
