...
- **Dock navy economy (DE):** shipyard trains fishing_ship + trade_cog; researches gillnets / shipwright. fish_trap (养鱼场, 100W, 715 food) built on water by fishing ships only. Gillnets: gather_time_fish_trap −20% (+25% work). Shipwright: ships wood −20% and time_cost −35% (~+54% create). trade_cog uses trade order between docks (same gold loop as carts).
...
- **Unique techs (full DE, not stand-ins):** Yasama ``rdg_seq_*`` (+2 arrows); Kataparuto fire rate + ``unpack_time -75%``; Crenellations range + ``passenger_attack_types infantry``; Chieftains cavalry/camel bonus + ``kill_resource_vs``; Paper Money ``gather_byproduct wood 0.014``; Circumnavigation ``reveal_map`` + ships ``time_cost -33%``; Arquebus ``projectile_lead`` + proj speed +0.5/+0.2.
