...
class GridView:
    ...
    def _is_object_visible(self, o):
        ...
        return (
            zoom.xmin <= o.x <= zoom.xmax
            and zoom.ymin <= o.y <= zoom.ymax
        )
    ...
    def _get_view_coords_from_world_coords(self, ox, oy):
        if self.interface.zoom_mode:
            zoom = self.interface.zoom
            x = int((ox - zoom.xmin) / (zoom.xmax - zoom.xmin) * get_screen().get_width())
            y = int((oy - zoom.ymin) / (zoom.ymax - zoom.ymin) * get_screen().get_height())
        else:
            sw = self.interface.world.square_width
            x = int(ox / sw * self.square_view_width)
            y = int(oy / sw * self.square_view_height)
        return x, y
    ...
    def display_object(self, o):
        ...
        # 获取或计算对象的渲染信息
        cache_key = (o.id, o.x, o.y)
        if cache_key in self._object_cache:
            x, y, color = self._object_cache[cache_key]
        else:
            x, y = self._get_view_coords_from_world_coords(o.x, o.y)
            ...
            # 缓存渲染信息
            self._object_cache[cache_key] = (x, y, color)
    ...
    def display_objects(self):
        ...
        for o in self.interface.dobjets.values():
            if self._is_object_visible(o):
                visible_objects.append(o)
                
        # 渲染可见对象
        for o in visible_objects:
            self.display_object(o)
            if (o.place is None and not o.is_inside and
                not (self.interface.already_asked_to_quit or self.interface.end_loop)):
