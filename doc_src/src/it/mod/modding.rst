...
Nel selettore di fazione le frecce leggono solo ``title``. Con ``intro``, premi **G** per un sottomenu frase per frase (da 1.4.7.2).
...
Flag di unità (0/1, ``int_properties``, predefinito 0): se i **proiettili a distanza predicono / colpiscono bersagli in movimento**.

- ``0``: il proiettile mira al punto al momento dello sparo; se il bersaglio si è allontanato all’impatto → miss
- ``1``: i proiettili possono predire i bersagli in movimento (stile «Balistica» di Age of Empires II)

**Richiede velocità di volo**: a distanza servono ``rdg_projectile`` + ``rdg_projectile_speed`` (**caselle/s**, >0). Altrimenti l’impatto è istantaneo e il controllo di miss per movimento non conta mai.

Questi campi sono **velocità**, non «secondi di volo». Esempio: ``7`` = sette caselle al secondo. Non inserire un ritardo di impatto desiderato (es. 0.57) nel campo.

**Non hardcodare il nome di una tecnologia nel motore.** Concedi il flag con un bonus di tecnologia, es.::

    effect info 8510
    effect bonus projectile_lead 1
    effect_bonus_targets archer_unit -hand_cannoneer galley scouttower

Le unità devono comunque avere la tecnologia in ``can_use_tech``. L’UI attributi non annuncia il ``+1`` di questo flag; usa ``effect info <tts_id>`` per un testo leggibile (vedi *info* sotto).

Distinto da ``rdg_projectile`` (regole di combattimento proiettile come portata da terreno alto). ``projectile_lead`` controlla solo il miss contro un bersaglio che si è mosso.

projectile_speed / mdg_projectile_speed / rdg_projectile_speed
==============================================================

**Velocità di volo** del proiettile per tipo di attacco in caselle/secondo (PRECISION), predefinito 0.

- ``rdg_projectile_speed``: solo per colpi **a distanza** quando ``rdg_projectile 1``
- ``mdg_projectile_speed``: solo per colpi di **proiettile in mischia** quando ``mdg_projectile 1``
- ``projectile_speed``: nome condiviso deprecato; al caricamento migra verso la/e corsia/e corrispondente/i

**Non** mettere qui «quanti secondi fino all’impatto» — è velocità. Il motore ricava l’arrivo come **distanza ÷ velocità** (più lontano = più tempo). Velocità 0 o attacco non-proiettile → impatto istantaneo.

Un’unità può avere sia ``mdg`` sia ``rdg`` (anche con lo stesso range): solo la corsia con ``*_projectile`` vola a quella velocità.

aoe2 circa DE (tutte **velocità**): frecce/torri ``7``, mangonel ``3.5``, trabucco ``1.6``.
...
Vedi anche: `Predizione proiettili e velocità di volo <projectile-lead.htm>`_.
...
Metti i filtri unità nella riga successiva come `effect_bonus_targets` (non sulla stessa riga di `effect bonus`)::
...
`effect_bonus_targets` si applica al `effect bonus` precedente (le unità devono comunque avere `can_use_tech`). Un `-` iniziale esclude una corrispondenza (come `phase_bonus_targets`), es. `effect_bonus_targets soldier -building`. L’alias `effect_targets` è accettato.

Almeno le seguenti proprietà dovrebbero funzionare: damage, armor, range, heal_level, speed, hp_max, hp.

- ``effect bonus hp_max N`` / ``hp_max N%``: aumenta solo il massimo; l’HP attuale non cambia.
- ``effect bonus hp N`` / ``hp N%``: aumenta HP attuale e ``hp_max`` della stessa quantità. Usare ``hp`` se la ricerca deve anche curare le unità vive.
food_cost e food_provided probabilmente non funzionano correttamente.

`projectile_lead` è un flag di combattimento 0/1; concedilo con `effect bonus projectile_lead 1` (vedi *projectile_lead* sopra). L’UI attributi omette il `+1` numerico di questo flag.
...
Testo solo visuale nella schermata attributi di tecnologia/abilità (nessun cambio di stats a runtime). Può essere combinato con altri effetti, es. Balistica: `effect info` più `effect bonus projectile_lead 1`.
...
- ``mdg`` / ``rdg``: danno base
- ``mdg_vs`` / ``rdg_vs``: bonus contro tipi di unità specifici. Valgono
  entrambe le forme (si possono mischiare; **le righe ripetute si uniscono**,
  quelle successive non sovrascrivono)::

      mdg_vs building 150 siege_unit 40
      mdg_vs building 150
      mdg_vs siege_unit 40

  Anche gli altri attributi ``*_vs`` a coppie (es. ``rdg_vs``, ``mdg_hit_rate_vs``,
  ``menace_vs``) accettano più coppie su una riga e uniscono le righe ripetute.
- ``mdf`` / ``rdf``: difesa
...
Penetrazione in linea (da regole, stile scorpione AoE2)
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

Non è la stessa cosa di ``mdg_piercing`` / ``rdg_piercing`` (percentuale di armatura ignorata): sono **colpi extra sulla linea di tiro**.

Il proiettile percorre il segmento dal tiratore al punto di mira e applica di nuovo il danno ai **nemici** vicini al segmento (escluso il primario). Se il primario manca, la penetrazione resta. Non colpisce gli alleati. I colpi extra sono in ordine dal più vicino al più lontano; il danno è il ``rdg`` / ``mdg`` di quella vittima (con ``*_vs``).
...
Stile trabucco AoE2: qualsiasi unità può attivarlo in ``rules.txt`` (nessun nome tipo fisso). Ordini UI: ``pack`` / ``unpack``.

======= ================ ============================================================
Campo   Tipo             Significato
======= ================ ============================================================
``packable`` 0/1         Abilita la modalità (serve un tempo positivo)
``unpack_time`` secondi  Tempo per spacchettare (dispiegare e sparare); PRECISION ms
``pack_time`` secondi    Tempo per impacchettare; se omesso usa ``unpack_time``
``spawn_packed`` 0/1     Nasce impacchettato (1, default) o già dispiegato (0)
``packed_mdf`` / ``packed_rdf``  Armatura opzionale da impacchettato
======= ================ ============================================================

Comportamento: impacchettato = muoversi senza attaccare; spacchettato = attaccare senza muoversi.
Muoversi impacchetta da solo; attaccare spacchetta da solo. **Stop** annulla subito.
Progresso: ``completeness,0..10`` → ``proportion_*`` (come l’addestramento).
...
- Danno, precisione (``mdg_hit_rate``/``rdg_hit_rate``, 0 = 100%%), cooldown
  (``*_cd``), wind-up (``mdg_ready``/``rdg_ready`` — non il ``*_delay`` balistico)
- HP (``hp`` corrente, altrimenti ``hp_max``), armatura (``max(mdf, rdf)``),
  schivata (``max(mdg_dodge_rate, rdg_dodge_rate)``)
- Portata d’attacco, velocità di movimento

Le vecchie chiavi ``mdg_cover`` / ``rdg_cover`` non sono alias (il ``cover`` di terreno non cambia).
Le vecchie chiavi ``mdg_dodge`` / ``rdg_dodge`` non sono alias (il ``dodge_vs`` di terreno e i suoni di schivata in style non cambiano).
...
``damage_seq mdg|rdg \<volte\> [(damage d1 d2 ...)] [(secondary pierce melee)] [(interval secondi)]``
...
- **Proiettili secondari (Chu Ko Nu di AoE2)**: ``(secondary 3 0)`` — il primo colpo = attacco
  live dell’unità (applicano gli upgrade) più melee fisso; i colpi successivi = pierce + melee fissi
  (**non** potenziati da fabbro/chimica). Il melee può essere ``0`` (danneggia ancora armature
  melee negative come gli arieti). La somma **non** deve eguagliare il danno base in questa modalità.
- Intervallo: ``(interval 0.25)`` secondi tra i colpi; se omesso o 0 con ``volte \> 1``,
  predefinito 0,25 s
- Limite: al massimo 6 colpi per attacco
- Tiraggi di colpo: ogni segmento tira indipendentemente colpo, critico e debuff; i colpi
  secondari colpiscono al 100%
- Cooldown: ``mdg_cd`` / ``rdg_cd`` inizia dopo che l’intera raffica è finita
  (prossima salva = ``(volte-1)×interval + cd``, poi ``ready``)
...
Chu Ko Nu di AoE2 DE (``mods/aoe2``; l’elite usa 5 frecce)::
...
| ``auto_production`` | Quando ``1``, mostra produzione automatica; ripete dopo ogni ciclo; per buffer (es. gold house). Il gas SC usa viaggi dei lavoratori (vedi Depositi e gas); non confondere con ``auto_cultivate`` delle fattorie |
...
Modalità di raccolta (da 1.4.6.9)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Sono disponibili due meccanismi di raccolta per le mod:

1. **``trip`` (predefinito):** un impulso ``gather_qty`` (più ``extraction_*``), poi consegna — viaggi stile StarCraft.
2. **``continuous``:** riempie ``carry_capacity`` a un ritmo al secondo, poi consegna — stile Age of Empires II/IV.
...
| --- | --- |
| ``gather_mode`` | ``trip`` (predefinito) o ``continuous``; su ``parameters`` e/o sul lavoratore |
| ``carry_capacity`` | limite di carico continuous; ``0`` torna a un ``gather_qty`` |
| ``carry_capacity_<type>`` | override per deposito/edificio |
| ``gather_rate`` / ``gather_rate_<type>`` | continuous risorse/s; altrimenti ``qty/time`` |

Usa ``effect bonus carry_capacity N`` per upgrade stile carriola. I bonus percentuali ``gather_time_*`` aumentano il tasso continuous effettivo. Le chiavi corrispondono al ``type_name`` del deposito (es. ``gather_time_food_livestock`` vs ``gather_time_food_carcass``), così i bonus pastori e cacciatori si possono separare senza hardcode dei nomi civ.

Sistema di mercato (da 1.4.6.9)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Compra/vendita, tributo e commercio di tratta sono guidati dalle regole — **senza** nomi di risorsa hardcoded né payout solo in oro. Tabelle complete: `Sistema di mercato <market-system.htm>`_.

Punti salienti:

- ``def parameters``: ``market_currency``, ``market_commodities``, ``market_menu_labels``, chiavi di tassa/tributo/distanza commerciale.
- Edifici: ``is_market 1``. Unità commerciali: ``is_trade_unit 1``, ``trade_hubs``, ``trade_rewards`` (una o più).
- Ordini: ``market_buy`` / ``market_sell`` / ``tribute`` / ``trade``.
- Esempio: ``mods/aoe2/rules.txt``; note per giocatori: `mercato e commercio <../player/market-and-trade.htm>`_.
...
| ``requires_build_field \<nome\>`` | Richiede quel campo per piazzare/costruire; ``0`` esenta il tipo (Nexus, Pylon, Assimilator; Photon Cannon **richiede** psi) |
...
Depositi e gas (``requires_deposit`` / ``is_an_extractor``)
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
| ``is_an_extractor 1`` | Al completamento, copia la riserva del deposito in ``source_qty``; i lavoratori la scalano raccogliendo |
| ``deposit_volume N`` | Riserva predefinita del deposito quando la mappa usa qty ``1`` |
| ``depleted_production_qty N`` | Resa per viaggio dopo ``source_qty`` a 0 (``0`` = stop; gas SC usa ``2``) |
| ``gather_slots N`` | Massimo lavoratori che estraggono insieme; ``0`` = illimitato. Gas SC usa ``3`` |

Il template gas ``sc_gas_building`` (stile SC1) usa ``is_an_extractor`` + raccolta a viaggio (``auto_production 0``) + ``extraction_qty`` / ``depleted_production_qty`` + ``gather_slots 3``.
I lavoratori necessitano di ``can_gather assimilator`` (tipo edificio), non ``geyser`` (deposito). Consiglio: **3** lavoratori per gas.
...
- I lavoratori Backspace/clic destro su ``is_huntable`` attaccano (l’attacco normale infligge danno); le uccisioni generano ``food_deposit`` (es. ``food_carcass``, o aoe2 ``food_livestock`` per ``herdable``) e completano l’ordine senza beep falso ``order_impossible``.
- Attr animali: ``is_huntable``, ``flee_on_hit``, ``agro_on_sight``, ``pursue_attacker``, ``herdable``, ``food_deposit``, ``food_deposit_qty``, ``no_number``.
...
- Velocità pastori / cacciatori separate: ``food_deposit`` distinti + ``gather_time_<deposito>`` (aoe2: britanni ``food_livestock``, mongoli ``food_carcass``). Guidato dalle rules, senza hardcode dei nomi civ.
- ``pursue_attacker 1``: dopo il contrattacco, insegue tra le caselle (attirare il cinghiale al centro in stile AoE2). Cervi/pecore usano ``flee_on_hit``.
- ``agro_on_sight``: default ``1``. ``0``: ``guard`` non spara a vista anche con ``formations 1``; contraattacca solo ``last_attacker`` e gli alleati non condividono l’aggro (cinghiale AoE2 DE). Predatori: ``1`` o omettere.
- ``pursue_leash_range N``: perde l’aggro se è oltre N mm dal bersaglio (``0`` = illimitato). I cinghiali usano ``48000`` (~4 caselle); poi dimenticano e tornano a casa.
- ``claimable 1``: da neutro, qualsiasi unità non neutrale vicina lo reclama (pecora AoE2). Indipendente da ``can_herd``.
- Pascolo: ``spawns_unit``, ``larva_spawn_time``, ``larva_cap``, ``spawn_player_cap`` / ``spawn_immediate`` (aoe2 ``pasture`` mongolo).
...
Con il display attivo, le mod possono fornire arte opzionale. Priorità sulla mappa:
``ui/anims/<tipo>/`` → ``ui/map/<set>/<tipo>.png`` → ``ui/map/<tipo>.png`` → forme geometriche.
I layer successivi coprono PNG omonimi. Formato: ``res/ui/anims/README.txt``.
Spine è opzionale; se manca, fallback a spritesheet/icone/forme. Codice: ``game_unit_anim.py``.

Set di architettura (da 1.4.8.3): ``ui/architecture.txt`` raggruppa le civ
(``def <set>`` + ``factions …``). aoe2 usa i set DE, non un miliziano per civ.
Giacimenti e fauna neutrale restano in cima. I RGB (``rim``, ecc.) solo per
``python tools/gen_aoe2_hud_icons.py``. Mod con tipi distinti per razza
(StarCraft: ``marine`` / ``zergling`` / ``zealot``) non servono sottocartelle.
...
Il testo deve esistere in ``tts.txt``. Anche le fazioni possono definire ``intro``: nel selettore premi **G** per un sottomenu frase per frase (da 1.4.7.2).
...
Formazioni (da 1.5.0.2)
>>>>>>>>>>>>>>>>>>>>>>>

Disattivate di default. Un mod con ``formations 1`` in ``def parameters`` dispone il gruppo militare in slot nel quadrato da ``class formation`` (AoE2 DE: linea, box, scalata, fianchi).
...
I nomi dei ranghi sono nelle rules: ``formation_ranks`` li elenca (si possono rinominare o aggiungere); ogni nome ha una tabella ``is_a`` ``formation_rank_<nome>``. Se ``formation_ranks`` è omesso, i nomi si inferiscono dalle chiavi ``formation_rank_*``. ``formation_default_rank`` se nulla coincide; ``formation_range_rank`` quando ``rdg_range`` è maggiore di ``mdg_range``; il fronte di combattimento è il primo rango di ``ranks`` della formazione attuale (o ``formation_front_rank``). I ranghi personalizzati usano ``title`` di style; ``melee`` / ``ranged`` / ``siege`` restano con TTS interno.

``shape``: cartesiane ``line`` / ``box`` / ``staggered`` / ``flank``, o polari ``ring`` / ``arc`` (alias ``circle`` / ``round`` → anello, ``wedge`` / ``cone`` → arco). Uno ``shape`` sconosciuto con ``radius`` / ``arc_span`` / ``rings`` usa comunque il packing polare, senza patch al motore. ``radius`` è in metri (0 = da ``spacing`` e dal conteggio). ``arc_span`` è in gradi (0 = 360 per l’anello, 180 per l’arco). ``arc_start`` di default è automatico. ``rings`` / ``ring_gap`` sono concentrici; ``ring_rank out`` mette il primo rango elencato all’esterno. Distanze in metri (PRECISION mm). ``flank_gap`` solo per i fianchi. ``max_front`` è il tetto per fila (0 = larghezza del quadrato). Un’unità può avere ``use_formation 1`` o ``formation_rank melee``; altrimenti valgono le liste ``is_a``. Il menu elenca ``set_formation <tipo>`` per ogni ``class formation`` (scegliere quello attuale riordina sul posto in ogni quadrato; selezionare tutto l’esercito non unisce quadrati lontani). ``cycle_formation`` resta fuori dal menu parlato; tasto predefinito ``CTRL SHIFT f``, modificabile nell’editor. Style: ``def set_formation`` e il ``title`` di ogni formazione; ``parameters.formation_change`` è l’SFX. Il ``go`` di gruppo verso un quadrato tiene i ranghi; ``go`` / ``attack`` su un’unità rompe gli slot e si concentra. L’``offensive`` inattivo all’ingaggio resta in formazione. Un nemico in formazione sul percorso intercetta (``formation_blocker``). Con le formazioni, ``guard`` è stai fermo: spara a tiro, non cammina (eccetto ``agro_on_sight 0``, cinghiale AoE2 DE). ``chase`` continua ad avvicinarsi. ``class formation`` può aggiungere ``mdg`` / ``rdg`` / ``mdf`` / ``rdf`` e ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs`` solo tenendo gli slot (si perdono con ``break_formation_hold`` / fuoco concentrato). ``speed`` (può essere negativo, es. ``speed -1.5`` o ``speed -30%``) si aggiunge dopo il tetto ``keep_pace``. Valori assoluti o percentuali della stat di quell’unità (``mdg 20%``). Default 0; i quattro tipi AoE2 restano solo layout. Con i ranghi tenuti, ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs`` coincidono anche con il nome ``class formation`` attuale dell’altro (es. ``formation_wedge``) e la forma (``cone`` / ``wedge`` / ``arc``, …), sommati al vs per tipo di unità; se coincidono più chiavi di forma, vale solo la più specifica. Si scrivono identificatori, non titoli tradotti. Nessun bonus se l’altro ha rotto i ranghi, insegue o non e ancora sullo slot (`formation_ranks_formed`). `keep_pace` resta camminando.
