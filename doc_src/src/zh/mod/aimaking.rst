...
- ``counter_skill \<0-100\>`` —— AI 单位利用 ``mdg_vs`` / ``rdg_vs`` 克制加成
  ...
  专家 ``90``、噩梦 ``100``。
- ``brain plan|adaptive|utility|tree`` —— 决策方式。``plan`` （默认）按剧本原顺序执行
  ``get``。``adaptive`` 仍完成同一行的全部 ``get`` 目标，但会根据侦察到的
  敌人，把 ``mdg_vs`` / ``rdg_vs`` 克制更好的兵种提前训练（工人/建筑相对顺序
  不变）；若该行已经在造兵，还会从**已有**兵营/靶场的 ``can_train`` 里最多补
  1 个克制兵种（纯农民开局行不补）。每拍跑一棵固定选择器树：回防 → 开局补农
...
- ``goto_random \<名称1\> \<名称2\> ...`` —— 在列出的标签中随机选一个跳转。
  非常适合让 AI 变得难以预测。
- ``if_enemy \<类型\> goto \<标签\>`` —— 感知或记忆中有匹配该类型（含
  ``is_a``）的敌人时跳转。
- ``if_not_enemy \<类型\> goto \<标签\>`` —— 尚未侦察到该类型时跳转。
- ``if_attacked goto \<标签\>`` —— 本拍有单位被攻击时跳转（与是否已还手无关）。
