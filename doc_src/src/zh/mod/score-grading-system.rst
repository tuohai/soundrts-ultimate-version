...
玩家向说明：`../player/score-and-grades.md <../player/score-and-grades.htm>`_

本文档说明 SoundRTS 对局结束后的多维度评分、字母评级与语音播报规则。实现位于 ``soundrts/worldplayerstats.py`` 的 ``Stats`` 类。

英文版：`../../en/mod/score-grading-system.htm <../../en/mod/score-grading-system.htm>`_

与成就系统的关系见 `achievement-system <achievement-system.htm>`_ 第 9 节；玩家简述见 `../player/score-and-grades <../player/score-and-grades.htm>`_。
...
.. list-table::
   ...
   * - ``base_total``
     - 七项基础维度之和，上限 800
   * - ``ai_defeat``
     - 击败敌方电脑额外加分，不计入 800 上限
   * - ``total``
     - `base_total + ai_defeat`，可超过 800
   * - ``percent``
     - `base_total × 100 ÷ 800`，上限 100%
   * - ``max``
     - 固定为 800（百分比分母，不含 ai_defeat）
   * - ``grade_total``
     - 用于字母评级的分数（败局有封顶，见 §5）
...
- 节约模式 `efficiency_mode = "frugal"`（仅胜利且利用率 < 50%）：
...
统计所有非己方、非同盟、非中立玩家的 ``produced(unit)`` 之和为 ``enemy_units``：
...
对每名已被击败的敌方电脑，按其 AI 难度累加 ``defeat_score``：
...
- 不计分的情况：同盟电脑、未击败、AI 类型为 ``timers`` / ``ai2`` / 空串、``defeat_score 0``、非电脑玩家。
...
.. list-table::
   ...
   * - ``soundrts/worldresource.py``
     - 地图 ``map_deposit_capacity`` 登记
   * - ``soundrts/worldunit/worldcreature.py``
     - ``lost`` / ``killed`` 统计
   * - ``soundrts/worldunit/world_attributes.py``
     - ``produced`` 统计
   * - ``soundrts/game.py``
     - `say_score()`、`post_run()`
