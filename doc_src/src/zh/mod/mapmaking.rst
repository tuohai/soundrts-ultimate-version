...
Building_land（建造用地默认类型）
''''''''''''''''''''''''''''''''''

自本版本起，地图可指定 ``nb_meadows_by_square`` 自动填充时使用的建造用地类型：
...
- ``building_land meadow``（默认）：自动填充 **草地**（``meadow``）。
- ``building_land build_site``：自动填充 **空地**（``build_site``），适用于太空/科幻等主题。

``additional_meadows`` 与 ``additional_build_sites`` 仍分别放置对应类型的对象；
``remove_meadows`` 仅移除 ``meadow``。

Nb_<type>_by_square（按规则中的建造用地类型）
''''''''''''''''''''''''''''''''''''''''''''''''''

地图关键字格式为 ``nb_<类型名>_by_square <数量>``，其中 ``<类型名>`` 必须是
``rules.txt`` 里 ``class building_land`` 的对象名（与 ``def`` 名称一致）::

    nb_build_site_by_square 1
    nb_meadow_by_square 2
    nb_volcanic_rock_by_square 1

- 自动在**每个格子**填充对应数量的该类型建造用地。
- 类型来自规则，不写死在引擎里；模组可自定义 ``def 火山岩石`` + ``class building_land``，
  地图即可写 ``nb_火山岩石_by_square 1``。
- 与 ``building_land`` 无关（各写各的类型）。
- 可与 ``nb_meadows_by_square`` 同时使用；一般只选其一。

遗留的 ``nb_meadows_by_square`` 仍保留：名称是历史习惯，实际类型由 ``building_land`` 决定
（默认 ``meadow``），不是 ``nb_meadow_by_square`` 的别名。

地图未写 ``building_land`` 且只使用一种 ``nb_<类型>_by_square`` 时，引擎会把该类型记为整张图的 ``world.building_land`` 默认。

起飞、部分升级在原地恢复建造用地时，优先使用**该建筑建造时保存的用地类型**；仅当建筑没有保存引用时，才用上述地图默认。
...
表示在 a2、b7 各添加 1 块 **build_site**（空地），不受 ``building_land`` 影响。

``speed_on_terrain``、单位在地形上的攻击/冷却/冲锋修正（``mdg_on_terrain`` 等，见 1.4.5.0）与地形/建造用地详见 ``building-land-terrain.htm``。
...
调色板 ``style`` 须与 ``rules.txt`` 里 ``class terrain`` 的地形名一致（如 ``forest``、``dense_forest``、``meadows``、``lake``）。应用时：

- **静态地形**（``is_dynamic 0``，如湖泊、高山）：锁定方格 ``type_name``，保存为 ``terrain <名>``。
- **动态地貌**（``is_dynamic 1``，如森林、密林、草地）：在格内放置 ``wood`` / ``meadow`` 等资源，由 ``square_terrain`` 解析地貌名；砍光树或摧毁建筑后地貌可消失。
