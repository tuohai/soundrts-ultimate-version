...
模组目录结构与 ``res`` 相同，可在 ``ui/`` 旁增加本地化文件夹（``ui-zh``、``ui-fr``、``ui-de`` 等）。游戏按用户 ``language.txt`` / 选项菜单语言（或 ``cfg/language.txt`` 回退、系统语言）加载对应语言；未翻译的条目回退到 ``ui/tts.txt``。
...
``space`` 为精度属性（可写小数），表示单位在同层（空/地/水）占一格地图的**抽象容量**。
容量与地图 ``square_width`` 同单位。不改变物理碰撞大小。

| 设置 | 效果 |
| --- | --- |
| ``space 0``（默认） | 不占抽象容量（兼容旧行为，该层容量视为无限） |
| ``square_width 12`` 下 ``space 1`` | 占用大小 12 中的 1（最多 12） |
| ``square_width 12`` 下 ``space 0.5`` | 最多 24 |
| ``square_width 12`` 下 ``space 12`` | 单独占满该格 |
| ``space`` > ``square_width`` | 无法进入该格 |

``square_width`` 就是每格大小（如 a1）。``space`` 与之同单位，仅作抽象容量记账。
容量**按同盟分别累计**（各方上限均为 ``square_width``）；敌军占用不占己方名额。同盟共享同一份名额。
本方名额已满时，无法再进入，也无法在此训练出新单位（语音 ``not_enough_space`` / 「空间不足」）。
...
- 其它命令会立刻顶掉正在执行的普通探索（回车启用）；Ctrl+回车启用则为强制探索，普通命令排在后面。开关仍开着。命令完成、单位再次空闲时自动恢复探索。
