...
- **问题**：aoe2 空城镇中心也会开火（写成建筑远程伤害），与决定版不符；条顿空中心 +5 箭、马里提圭 +8 箭无法用规则表达。
- **改进**：``garrison_arrows 1`` 时箭数 = ``base_arrows`` + 驻军中可开火单位，上限 ``max_garrison_arrows``（未写则 10）。空中心 ``base_arrows 0`` 不打；条顿 ``base_arrows 5``；提圭用已有 ``effect bonus base_arrows 8``。箭是建筑齐射，不是乘客各自射击。引擎不检查文明名。
- **范围**：全部模组；aoe2 城镇中心启用。城堡、箭塔仍可空建筑开火。

**aoe2：马里（Malians）**

- **问题**：模组没有马里文明。
- **改进**：第十三文明。建筑除农田外木费 −15%；兵营民兵/枪兵线封建/城堡/帝王 +1/+2/+3 远程护甲（不含女卫兵）；村民金矿卸货 +10%；组队大学科技研究快 80%（``team_on_phase`` + ``time_cost -44%``）。特色单位女卫兵；城堡科技提圭（200 食物 300 木材，城镇中心 ``base_arrows`` +8）；帝王科技法林巴（骑兵近战 +5）。选文明简介 ``8532``。
- **范围**：aoe2 模组。
