狩猎系统说明
============


SoundRTS 提供帝国时代风格的狩猎：村民攻击野生动物，击杀后在原地留下可采集的食物尸体，并可驱赶羊群跟随。


----


1. 玩家流程
-----------


1. 退格 / 默认命令或右键动物 → ``go``（靠近 / 归属），**己方牲畜也一样**；有 ``can_herd`` 的工人对 ``herdable`` 仍默认 ``herd``。要**攻击**（打猎 / 屠宰）须用强制命令（Ctrl+退格，或选 ``go`` 后 Ctrl+回车）
2. 击杀后 → 按动物 ``food_deposit`` 生成尸体矿床（原版多为 ``food_carcass``；aoe2 羊为 ``food_livestock``）；攻击命令标记完成，**不会**误播 ``order_impossible``
3. 自动采集 → 工人击杀后可自动排队采集尸体；若开启 ``auto_gather``，也会采集并运回仓库
4. 受击逃跑 → 鹿、羊被攻击后会逃离；野猪 ``agro_on_sight 0``，未被打不主动咬人，被打才反击并跨格追击（``pursue_attacker``，可诱到城镇中心；拉开 ``pursue_leash_range`` 会脱仇）
5. 靠近归属（可选）→ 任意非中立单位靠近中立 ``claimable`` 动物即归属（帝国 2 式领羊），并播放确认音效与「羊 , 已归属」类提示；``can_herd`` 仍是单独的驱赶跟随
6. 牧羊（可选）→ 配置了 ``can_herd 1`` 的工人可驱赶 ``herdable`` 动物（如羊）
7. 草场（可选）→ 建筑配置 ``spawns_unit`` / ``spawn_player_cap``（aoe2 蒙古 ``pasture``）周期性刷出己方牲畜；``spawn_immediate 1`` 建成时只出 **1** 只，之后按间隔补到同格 ``larva_cap``。蒙古牧民不建磨坊/农田，草场只需城镇中心，并可存放食物


说明：对**所有中立单位**（野生动物、creep、NPC）默认命令都是 ``go``（移动 / 靠近）。进攻 / 防御 / 追击 AI **不会**自动攻击中立单位，除非下达强制攻击。


----


2. 语音播报与「动物」标识
-------------------------


狩猎动物虽由 ``computer_only ... neutral`` 放置，但不会播报为「中立 NPC」。


.. list-table::
   :header-rows: 1

   * - 场景
     - 播报示例
   * - 选中鹿
     - 鹿 , 动物
   * - 方格摘要
     - , 2鹿 , 动物
   * - Ctrl+Shift+F4 切到纯动物玩家
     - 你是动物



识别规则：

- 单位配置了 ``is_huntable 1`` 或 ``herdable 1`` → 视为野生动物，播报「动物」
- 名称与「动物」之间用逗号隔开（与敌人、盟友等播报格式一致）
- Ctrl+Shift+F4 仅当该玩家所有存活单位均为野生动物时播报「你是动物」；混合 ``quest_npc`` 与鹿的玩家仍播报「你是中立 NPC」

剧情 NPC（``quest_npc`` 等）保持「中立 , NPC」播报，与狩猎动物区分。


----


3. 地图放置
-----------


在地图文件中使用中立电脑玩家放置：

.. code-block:: text

   computer_only 0 0 neutral b3 4 deer 2 sheep


随机地图会在出生点附近自动生成浆果丛（``orchard``）与野生动物。

3.1 外交：动物不是联盟成员
~~~~~~~~~~~~~~~~~~~~~~~~~~


狩猎动物由 ``computer_only`` 生成，但不会加入默认的 ``ai`` 电脑联盟，也不能与玩家或其它势力结盟。


.. list-table::
   :header-rows: 1

   * - 规则
     - 说明
   * - 识别条件
     - 该 ``computer_only`` 槽位里只有配置了 ``is_huntable 1`` 或 ``herdable 1`` 的单位（如鹿、羊、自定义老虎）
   * - 引擎行为
     - 该槽位对应电脑的 `alliance = None`，``allied`` 仅含自己
   * - 多群动物
     - 每一行 ``computer_only`` 是独立狩猎点，彼此之间也不结盟
   * - 混编槽位
     - 同一行若既有动物又有步兵等，整槽仍按普通电脑处理，会进 ``ai`` 联盟
   * - 玩家外交
     - 中立玩家不可 F12 结盟；狩猎动物更不会被当作可结盟势力



自定义动物示例（会按动物隔离，不进 ``ai`` 联盟）：

.. code-block:: text

   def tiger
   class soldier
   is_huntable 1
   ...
   
   computer_only 0 0 neutral 5,5 2 tiger


若要做「多群动物联合成一个自然阵营」，需用触发器 ``(alliance …)`` 显式设置，不属于默认狩猎行为。


----


4. rules.txt 配置
-----------------


4.1 内置单位
~~~~~~~~~~~~



.. list-table::
   :header-rows: 1

   * - 类型
     - 说明
   * - ``deer`` （鹿）
     - 35 食物，受击逃跑；``food_deposit food_carcass``
   * - ``sheep`` （羊）
     - 可驱赶，受击逃跑；原版 ``food_carcass``，aoe2 ``food_livestock``
   * - ``boar`` （野猪）
     - 会反击并跨格追击（``pursue_attacker``，可诱到城镇中心）；``food_deposit food_carcass``
   * - ``food_carcass``
     - 狩猎尸体矿床（``collision 0``，不占格）
   * - ``food_livestock``
     - aoe2 畜牧尸体矿床（牧羊加成针对此类型）
   * - ``orchard``
     - 浆果丛/果园矿床，采集后运回基地计入食物



村民 ``can_gather`` / ``can_gather_deposit`` 应列出可采的尸体类型（如 ``food_carcass``，aoe2 另加 ``food_livestock``），以及用到的 ``orchard``。

**牧羊 / 打猎分开加成（规则驱动）：** 给不同动物写不同 ``food_deposit``，再用 ``on_phase`` / 科技 ``gather_time_<矿床>``（例：不列颠 ``gather_time_food_livestock``，蒙古 ``gather_time_food_carcass``）。引擎按矿床 ``type_name`` 匹配加成；AI 能否狩猎由 ``is_huntable`` 动物的 ``food_deposit`` 推导，代码不写死文明名。

4.1.1 储量播报名与入库资源
~~~~~~~~~~~~~~~~~~~~~~~~~~


狩猎尸体在地图上仍按来源动物播报物体名（如「鹿的尸体」）；果园等矿床用 ``style.txt`` 里的 ``title`` （如「果园」）。

「含有 50 ___」中数量后面的词由 ``style.txt`` 的 ``qty_unit_title`` 配置（尸体矿床默认「肉」、果园默认「果子」），与入库后玩家资源栏读法分开：

- `鹿的尸体 , 向东 , 含有 50 肉`
- `果园 , 向北 , 含有 50 果子`

运回含 ``resource3`` 的仓库后，资源栏与 Shift+Z 仍播报食物（``resource3_title``），由 rules 的 ``resource_type resource3`` 决定。

.. code-block:: text

   def food_carcass
   class deposit
   resource_type resource3

   def food_livestock
   class deposit
   resource_type resource3
   
   def orchard
   class deposit
   resource_type resource3


在 ``ui/style.txt`` 中可为矿床类型设置 ``qty_unit_title`` （对应 ``ui-zh/tts.txt`` 文案），仅影响「含有 N …」的播报，不改变尸体/果园物体名本身。

4.2 动物属性
~~~~~~~~~~~~



.. list-table::
   :header-rows: 1

   * - 属性
     - 说明
   * - ``is_huntable 1``
     - 可被村民狩猎；右键默认攻击
   * - ``agro_on_sight``
     - 默认 ``1``。``0``：未被打不主动攻击、不受友军连坐（决定版野猪）；掠食者写 ``1``
   * - ``flee_on_hit 1``
     - 受击后向远离攻击者方向逃跑
   * - ``pursue_attacker 1``
     - 反击后跨格追击攻击者（野猪诱敌到城镇中心）
   * - ``pursue_leash_range``
     - 与追击目标的最大距离（毫米）；超出则忘仇并回出生点（``0``=不限制；野猪 ``48000``≈4 格）
   * - ``herdable 1``
     - 可被 ``can_herd`` 工人驱赶跟随
   * - ``claimable 1``
     - 仍为中立时，附近任意非中立单位靠近即归属（帝国 2 领羊）；与 ``can_herd`` 驱赶机制独立
   * - ``claim_range``
     - 归属距离（毫米；``0``=仅同格；羊用 ``4000``≈帝国 2 DE 4 格；判定含双方碰撞半径）
   * - ``herd_leash_range``
     - 驱赶跟随的最大距离（毫米）
   * - ``food_deposit``
     - 死亡后生成的食物矿床类型名
   * - ``food_deposit_qty``
     - 尸体食物储量（游戏内单位，如 35 = 35 食物）
   * - ``no_number 1``
     - 同类型仅 1 个时不报序号（适合动物）



工人侧：


.. list-table::
   :header-rows: 1

   * - 属性
     - 说明
   * - ``can_herd 1``
     - 可驱赶 ``herdable`` 动物；默认 `0`



4.3 自定义动物示例
~~~~~~~~~~~~~~~~~~


.. code-block:: text

   def wolf
   class soldier
   is_huntable 1
   flee_on_hit 1
   food_deposit food_carcass
   food_deposit_qty 40
   no_number 1
   ai_mode guard
   ...


4.4 狩猎科技
~~~~~~~~~~~~


``hunting_techniques`` （狩猎技术）：缩短尸体/浆果采集时间，提高尸体采集量，并增加动物尸体储量。可在市政厅研究。


----


5. 与中立 NPC 的区别
--------------------



.. list-table::
   :header-rows: 1

   * - 
     - 狩猎动物
     - 剧情 NPC
   * - 典型单位
     - ``deer``、``sheep``、``boar``
     - ``quest_npc``、``npc_knight``
   * - 识别属性
     - ``is_huntable`` / ``herdable``
     - 无（可有 ``receive_items``）
   * - 语音归属
     - 动物
     - 中立 , NPC
   * - 玩家主动攻击
     - 需强制攻击（与中立 creep 相同）
     - 需强制攻击



详见 `单位默认模式与自动状态配置说明 <单位默认模式与自动状态配置说明.htm>`_。


----


6. 代码与测试
-------------



.. list-table::
   :header-rows: 1

   * - 角色
     - 路径
   * - 狩猎逻辑
     - ``soundrts/worldunit/worldcreature.py``、``worldworker.py``
   * - 动物外交隔离
     - ``soundrts/worldplayerbase/base.py`` （``computer_start_is_wildlife_only``）、``world_objects.py``
   * - 动物播报
     - ``soundrts/clientgameentity/properties.py`` （``is_wildlife_unit``）
   * - 切视角播报
     - ``soundrts/clientgame/game_resources.py`` （``cmd_change_player``）
   * - 随机地图生成
     - ``soundrts/randommap.py``
   * - 测试
     - ``soundrts/tests/test_hunting.py``、``test_wildlife_identification.py``、``test_wildlife_alliance.py``

