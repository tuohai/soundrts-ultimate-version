...
- 对**所有中立单位**（野生动物 / creep / NPC）默认命令是 ``go``（靠近 / 归属）；普通 ``go`` 只移动，不挂攻击动作；
- 要**攻击**中立单位（打猎、杀牲畜、打 creep）须下达强制攻击（``imperative``：
  例如 Ctrl+退格，或选 ``go`` 后 Ctrl+回车；引擎会把强制 ``go`` 转为 ``attack``）；
- 已归属（非中立）的 ``is_huntable`` 动物默认仍是 ``attack``（屠宰）。
...
- 中立与站岗绑定：中立电脑保持 ``guard``；一旦 UI 或 ``set_ai_mode`` 切到
  ``offensive`` / ``defensive`` / ``chase``，会自动解除中立变为正式敌对
  （纯野生动物电脑除外）。切回站岗不会自动恢复中立；可用 ``(set_neutral 1)``。
- 中立 creep **遭到**非中立攻击时也会解除中立（与切模式互补），便于己方
  自动交战；野生动物除外。
...
- 持续状态：开启后，单位每次变空闲都会重新开始探索（沿用启用时的普通/强制模式）。
- 被命令打断：普通探索会被其它命令立刻顶掉；强制探索时普通命令排在后面。开关仍开着。该命令完成、单位再次
  空闲时，会自动恢复探索。
- 彻底停止：使用 "禁用自动探索" 命令即可关闭。该命令只要单位当前正处于自动探索
  状态就会出现（即便没配 ``can_auto_explore``），避免"开着却关不掉"。
- 重新启用："启用自动探索"命令只对配置了 ``can_auto_explore 1`` 的单位出现。回车为普通，Ctrl+回车为强制。
