...
- Animal attrs: ``is_huntable``, ``flee_on_hit``, ``agro_on_sight``, ``pursue_attacker``, ``herdable``, ``claimable``, ``claim_range``, ``food_deposit``, ``food_deposit_qty``, ``no_number``.
...
- ``agro_on_sight``: default ``1``. ``0`` keeps ``guard`` from stand-ground fire even with ``formations 1``; only ``last_attacker`` is fought, and allies do not share aggro (AoE2 DE boar). Predators set ``1`` or omit it.
