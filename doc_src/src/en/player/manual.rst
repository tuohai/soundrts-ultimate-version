...
- **Map sprites**: the top-down map uses ``res/ui/map/<type>.png`` (separate from HUD ``ui/icons``); otherwise colored circles/squares. When many units share a square the sprites shrink and a team-color pip marks the true position.
- **Optional unit animation**: put packs under ``res/ui/anims/<type>/`` (spritesheet, or optional Spine). Priority: anim → ``ui/map`` → shapes. Starter geometric sheets: ``python tools/gen_unit_anims.py`` (4-dir idle/walk/attack/gather). See ``res/ui/anims/README.txt``.
...
- **Mod art** (since 1.4.8.3): ``mods/<name>/ui/icons`` and ``ui/map`` overlay same-named base PNGs. aoe2 also uses architecture-set subfolders from ``ui/architecture.txt`` (e.g. ``western_european/militia.png``); StarCraft uses one PNG per type name.
