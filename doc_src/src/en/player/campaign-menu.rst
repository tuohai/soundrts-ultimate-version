...
Chinese version: `../../zh/player/战役与合作战役改进说明 <../../zh/player/战役与合作战役改进说明.htm>`_.
...
.. list-table::
   ...
   * - Area
     ...
     - Co-op
   * - Menu
     - Mission browser: synopsis, difficulty, completed/locked
     - Server: campaign → chapter → difficulty → speed
   * - Difficulty
     - Five tiers, saved in ``user/campaigns.ini``
     - Same + extra scaling by human player count
   * - Enemy scaling
...
Core code: `soundrts/coop_difficulty.py <../../../soundrts/coop_difficulty.py>`_, ``soundrts/campaign.py <../../../soundrts/campaign.py>`_, ``soundrts/clientservermenu.py <../../../soundrts/clientservermenu.py>`_, ```soundrts/serverroom.py``.
...
Progress is stored in `user/campaigns.ini <../../../soundrts/paths.py>`_ (``chapter`` + ``difficulty`` per campaign id).

2.1.1 Hero growth and cross-mission carryover (rules-driven)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


Configure any hero in :strong:```rules.txt`` with ``campaign_carryover 1`` (not Raynor-specific).
...
Author guide: `../mod/campaign-hero-carryover <../mod/campaign-hero-carryover.htm>`_.
...
Example: ``res/single/The Legend of Raynor/campaign.txt``.
...
- Authoring: `coop-campaign.md <../mod/coop-campaign.htm>`_ and §4 below.
...
Example map: ``res/single/The Legend of Raynor/1.txt``.
...
.. list-table::
   ...
   * - ``nb_players_max``
     - Co-op slot count
   * - ``nb_players_min 1``
     - Solo + AI partners allowed
   * - ``player_start N …``
     - Spawn square and units for slot N
   * - ``computer_only``
     - Mission enemies (`"ai"` alliance vs humans on alliance 1)



``Game._fill_coop_ai_partners`` fills empty slots with aggressive allied AI; all humans + partners start on alliance 1.  
...
Logical map name ``CampaignName/chapter`` triggers `apply_campaign_from_map_name <../../../soundrts/lib/resource.py>``_ so ``rules.txt`` and campaign ``tts.txt`` load on clients; square names like ``loc_ch02_*`` resolve through TTS instead of reading raw keys.
...
Declare co-op like Age of Empires in :strong:```campaign.txt``. Do not ship parallel ``N.coop.txt``
...
.. list-table::
   ...
   * - Field
     - Meaning
   * - ``coop_campaign``
     - ``1`` — show in server Co-op campaign menu
   * - ``coop_intro``
     - Cutscene chapter numbers in the co-op flow (e.g. prologue `0`)
   * - ``coop_missions``
     - Mission chapters playable in co-op (`1-29`, space lists, etc.)
...
.. list-table::
   ...
   * - Old
     - New
   * - Chapter list only
     - Synopsis + difficulty + completed/locked + retry
   * - Co-op without difficulty / treaty step
     - Five tiers + count scaling; no treaty
   * - Co-op as skirmish
     - Shared intro/cutscenes/objectives; AI partners
   * - ``N.coop.txt`` or file-based co-op detection
     - ``campaign.txt`` flags + shared ``N.txt``
   * - Raw `loc_*` keys in co-op
     - Campaign TTS layer, localized names
   * - Standard = baseline
     - Still 100%/100%; other tiers per table
...
.. list-table::
   ...
   * - [campaign-northern-arc.htm](campaign-secret-letter-alliance.htm)
     - The Legend of Raynor ch. 24–27
   * - [coop-campaign.md](coop-campaign.htm)
     - Short co-op reference
   * - ``mod/mapmaking.rst``
     - Mission syntax
...
.. list-table::
   ...
   * - ``soundrts/campaign.py``
     - SP browser, co-op metadata (`coop_*`), bookmarks, difficulty
   * - ``soundrts/coop_difficulty.py``
     - Tier table and count multiplier
   * - ``soundrts/clientservermenu.py``
     - Co-op menu, ``srv_coop_difficulty``
   * - ``soundrts/serverroom.py``
     - AI partners, difficulty broadcast
   * - ``soundrts/game.py``
     - ``is_coop_campaign``, intro, bookmark update
   * - ``soundrts/worldunit/worldcreature.py``
     - Enemy HP scale
   * - ``soundrts/combat/damage_effects.py``
     - Enemy damage scale
   * - ``soundrts/lib/resource.py``
     - Campaign resource stack, place TTS
