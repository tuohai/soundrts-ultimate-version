...
En el selector de facción, las flechas solo leen ``title``. Con ``intro``, pulse **G** para un submenú frase a frase (desde 1.4.7.2).
...
Bandera de unidad (0/1, ``int_properties``, predeterminado 0): si los **proyectiles a distancia predicen / aciertan objetivos en movimiento**.

- ``0``: el proyectil apunta al punto del momento del disparo; si el objetivo se ha alejado al impactar → fallo
- ``1``: los proyectiles pueden predecir objetivos en movimiento (estilo «Balística» de Age of Empires II)

**Requiere velocidad de vuelo**: a distancia hace falta ``rdg_projectile`` + ``rdg_projectile_speed`` (**casillas/s**, >0). Si no, el impacto es instantáneo y la comprobación de fallo por movimiento nunca importa.

Estos campos son **velocidad**, no «segundos de vuelo». Ejemplo: ``7`` = siete casillas por segundo. No ponga un retraso de impacto deseado (p. ej. 0.57) en el campo.

**No hardcodee el nombre de una tecnología en el motor.** Otorgue la bandera con un bonus de tecnología, p. ej.::

    effect info 8510
    effect bonus projectile_lead 1
    effect_bonus_targets archer_unit -hand_cannoneer galley scouttower

Las unidades aún necesitan la tecnología en ``can_use_tech``. La UI de atributos no anuncia el ``+1`` de esta bandera; use ``effect info <tts_id>`` para un texto legible (véase *info* más abajo).

Distinto de ``rdg_projectile`` (reglas de combate de proyectil como alcance en terreno alto). ``projectile_lead`` solo controla el fallo frente a un objetivo que se movió.

projectile_speed / mdg_projectile_speed / rdg_projectile_speed
==============================================================

**Velocidad de vuelo** del proyectil por tipo de ataque en casillas/segundo (PRECISION), predeterminado 0.

- ``rdg_projectile_speed``: solo para golpes **a distancia** cuando ``rdg_projectile 1``
- ``mdg_projectile_speed``: solo para golpes de **proyectil cuerpo a cuerpo** cuando ``mdg_projectile 1``
- ``projectile_speed``: nombre compartido obsoleto; al cargar se migra a la(s) vía(s) correspondiente(s)

**No** ponga aquí «cuántos segundos hasta el impacto» — es velocidad. El motor calcula la llegada como **distancia ÷ velocidad** (más lejos = más tarda). Velocidad 0 o ataque no proyectil → impacto instantáneo.

Una unidad puede tener ``mdg`` y ``rdg`` a la vez (incluso el mismo alcance): solo la vía con ``*_projectile`` vuela a esa velocidad.

aoe2 aprox. DE (todas **velocidades**): flechas/torres ``7``, mangonel ``3.5``, trabuquete ``1.6``.
...
Véase también: `Predicción de proyectiles y velocidad de vuelo <projectile-lead.htm>`_.
...
Ponga los filtros de unidad en la línea siguiente como `effect_bonus_targets` (no en la misma línea que `effect bonus`)::
...
`effect_bonus_targets` se aplica al `effect bonus` precedente (las unidades aún necesitan `can_use_tech`). Un `-` inicial excluye una coincidencia (igual que `phase_bonus_targets`), p. ej. `effect_bonus_targets soldier -building`. Se acepta el alias `effect_targets`.

Al menos las siguientes propiedades deberían funcionar: daño, armadura, alcance, nivel de curación, velocidad, hp_max, hp.

- ``effect bonus hp_max N`` / ``hp_max N%``: solo sube el máximo; el HP actual no cambia.
- ``effect bonus hp N`` / ``hp N%``: sube HP actual y ``hp_max`` en la misma cantidad. Use ``hp`` si la investigación también debe curar unidades vivas.
food_cost y food_provided probablemente no funcionen correctamente.

`projectile_lead` es una bandera de combate 0/1; otórguela con `effect bonus projectile_lead 1` (véase *projectile_lead* arriba). La UI de atributos omite el `+1` numérico de esta bandera.
...
Texto solo de visualización en la pantalla de atributos de tecnología/habilidad (sin cambio de stats en tiempo de ejecución). Puede combinarse con otros efectos, p. ej. Balística: `effect info` más `effect bonus projectile_lead 1`.
...
- ``mdg_vs`` / ``rdg_vs``: bonificación frente a tipos de unidades específicas.
  Cualquiera de las dos formas vale (se pueden mezclar; **varias líneas se
  fusionan**, la posterior no sobrescribe la anterior)::

      mdg_vs building 150 siege_unit 40
      mdg_vs building 150
      mdg_vs siege_unit 40

  Otros atributos ``*_vs`` por pares (p. ej. ``rdg_vs``, ``mdg_hit_rate_vs``,
  ``menace_vs``) también aceptan varios pares en una línea y fusionan líneas
  repetidas.
- ``mdf`` / ``rdf``: defensa
...
- ``mdg_projectile_speed`` / ``rdg_projectile_speed``: velocidad de vuelo del proyectil por vía (casillas/s; sustituye delay / ``projectile_speed`` compartido)
...
- ``rdg_bounce`` / ``mdg_bounce``, ``*_bounce_range``, ``*_bounce_decay``: rebote a enemigos cercanos tras un impacto (véase abajo; estilo Mutalisk). No es splash ni penetración en línea
...
- ``rdg_pierce_decay`` / ``mdg_pierce_decay``: porcentaje de daño extra tras la armadura; 0 u omitido = 100. Escorpión AoE2 usa 50
...
Rebote (por reglas, estilo Mutalisk de StarCraft)
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

No es lo mismo que el splash circular (``rdg_splash``) ni la penetración en línea (``rdg_pierce_line``): después del **impacto principal**, el disparo salta a otro enemigo cercano.

Cada salto elige al enemigo más cercano que esta cadena aún no haya golpeado y que coincida con ``rdg_targets`` / ``mdg_targets``. No hiere aliados. Si el primario falla, no hay rebote. El daño es el ``rdg`` / ``mdg`` de esa víctima (con ``*_vs``), reducido en cada salto.

- ``rdg_bounce`` / ``mdg_bounce``: saltos extra; 0 u omitido = desactivado. Mutalisk usa 2 (tres objetivos en total)
- ``rdg_bounce_range`` / ``mdg_bounce_range``: distancia máxima por salto (casillas); omitido o 0 usa ``rdg_range`` / ``mdg_range``
- ``rdg_bounce_decay`` / ``mdg_bounce_decay``: porcentaje del salto anterior que se conserva (redondeo al más cercano); omitido o 0 = 33 (9→3→1)

Ejemplo (mutalisk de StarCraft)::
...
Estilo trabuquete de AoE2: cualquier unidad puede activarlo en ``rules.txt`` (sin nombres de tipo fijos). En la UI: órdenes ``pack`` / ``unpack``.

======= ================ ============================================================
Campo   Tipo             Significado
======= ================ ============================================================
``packable`` 0/1         Activa el modo (hace falta un tiempo positivo)
``unpack_time`` segundos Tiempo para desempacar (desplegar y disparar); PRECISION ms
``pack_time`` segundos   Tiempo para empacar; si se omite, usa ``unpack_time``
``spawn_packed`` 0/1     Nace empacado (1, por defecto) o ya desplegado (0)
``packed_mdf`` / ``packed_rdf``  Armadura opcional mientras está empacado
======= ================ ============================================================

Comportamiento: empacado = moverse sin atacar; desempacado = atacar sin moverse.
Mover empaca solo; atacar desempaca solo. **Stop** cancela al instante.
Progreso: ``completeness,0..10`` → ``proportion_*`` (como el entrenamiento).
...
- Daño, acierto (``mdg_hit_rate``/``rdg_hit_rate``, 0 = 100%%), enfriamiento
...
Las claves antiguas ``mdg_cover`` / ``rdg_cover`` no son alias (el ``cover`` de terreno no cambia).
Las claves antiguas ``mdg_dodge`` / ``rdg_dodge`` no son alias (el ``dodge_vs`` de terreno y los sonidos de esquiva de style no cambian).
...
- **Proyectiles secundarios (Chu Ko Nu de AoE2)**: ``(secondary 3 0)`` — el primer disparo = ataque
  vivo de la unidad (aplican mejoras) más melee fijo; disparos posteriores = pierce + melee fijos
  (**no** mejorados por herrería/química). El melee puede ser ``0`` (aún daña armadura melee
  negativa como arietes). La suma **no** necesita igualar el daño base en este modo.
...
- Tiradas de golpe: cada segmento tira golpe, crítico y desventaja por separado; los disparos
  secundarios aciertan al 100%
- Enfriamiento: ``mdg_cd`` / ``rdg_cd`` comienza después de que termina la ráfaga completa
  (siguiente volea = ``(times-1)×interval + cd``, luego ``ready``)
...
Chu Ko Nu de AoE2 DE (``mods/aoe2``; la élite usa 5 flechas)::
...
| ``auto_production`` | Cuando ``1``, muestra producción automática; bucles después de cada ciclo; uso para buffers (p. ej. gold house). El gas SC usa viajes de trabajadores (véase Depósitos y gas); no confundir con ``auto_cultivate`` de granjas |
...
Modos de recolección (desde 1.4.6.9)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Hay dos mecanismos de recolección disponibles para mods:

1. **``trip`` (predeterminado):** un pulso ``gather_qty`` (más ``extraction_*``), luego entregar — viajes estilo StarCraft.
2. **``continuous``:** llena ``carry_capacity`` a un ritmo por segundo, luego entrega — estilo Age of Empires II/IV.
...
| --- | --- |
| ``gather_mode`` | ``trip`` (predeterminado) o ``continuous``; en ``parameters`` y/o en el trabajador |
| ``carry_capacity`` | límite de carga continuous; ``0`` vuelve a un ``gather_qty`` |
| ``carry_capacity_<type>`` | anulación por depósito/edificio |
| ``gather_rate`` / ``gather_rate_<type>`` | continuous recursos/s; si no, ``qty/time`` |

Use ``effect bonus carry_capacity N`` para mejoras estilo carretilla. Los bonus porcentuales ``gather_time_*`` aumentan la tasa continuous efectiva. Las claves coinciden con el ``type_name`` del depósito (p. ej. ``gather_time_food_livestock`` frente a ``gather_time_food_carcass``), así los bonus de pastores y cazadores pueden separarse sin hardcodear civs.

Sistema de mercado (desde 1.4.6.9)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Compra/venta, tributo y comercio de rutas son impulsados por reglas — **sin** nombres de recurso hardcodeados ni pagos solo en oro. Tablas completas: `Sistema de mercado <market-system.htm>`_.

Aspectos destacados:

- ``def parameters``: ``market_currency``, ``market_commodities``, ``market_menu_labels``, claves de impuesto/tributo/distancia de comercio.
- Edificios: ``is_market 1``. Unidades de comercio: ``is_trade_unit 1``, ``trade_hubs``, ``trade_rewards`` (uno o más).
- Órdenes: ``market_buy`` / ``market_sell`` / ``tribute`` / ``trade``.
- Cableado de ejemplo: ``mods/aoe2/rules.txt``; notas para jugadores: `mercado y comercio <../player/market-and-trade.htm>`_.
...
| ``requires_build_field \<name\>`` | Requiere ese campo para colocar/construir; ``0`` exime el tipo (Nexus, Pylon, Assimilator; Photon Cannon **sí** necesita psi) |
...
| ``is_an_extractor 1`` | Al completar, copia la reserva del depósito a ``source_qty``; los trabajadores la descuentan al recolectar |
| ``deposit_volume N`` | Reserva por defecto del depósito cuando el mapa usa qty ``1`` |
| ``depleted_production_qty N`` | Rendimiento por viaje tras vaciar ``source_qty`` (``0`` = parar; gas SC usa ``2``) |
| ``gather_slots N`` | Máximo de trabajadores extrayendo a la vez; ``0`` = sin límite. Gas SC usa ``3`` |

La plantilla de gas ``sc_gas_building`` (estilo SC1) utiliza ``is_an_extractor`` + recolección por viaje (``auto_production 0``) + ``extraction_qty`` / ``depleted_production_qty`` + ``gather_slots 3``.
Los trabajadores necesitan ``can_gather assimilator`` (tipo de edificio), no ``geyser`` (depósito). Consejo: **3** trabajadores por gas.
...
- Los trabajadores Retroceso/clic derecho sobre ``is_huntable`` atacan (el ataque normal hace daño); las muertes generan ``food_deposit`` (p. ej. ``food_carcass``, o aoe2 ``food_livestock`` para ``herdable``) y completan la orden sin pitido falso ``order_impossible``.
- Atributos animales: ``is_huntable``, ``flee_on_hit``, ``agro_on_sight``, ``pursue_attacker``, ``herdable``, ``food_deposit``, ``food_deposit_qty``, ``no_number``.
...
- Velocidad de pastores / cazadores por separado: distintos ``food_deposit`` + ``gather_time_<depósito>`` (aoe2: britanos ``food_livestock``, mongoles ``food_carcass``). Por reglas, sin hardcodear civs.
- ``pursue_attacker 1``: tras contraatacar, perseguir entre casillas (atraer jabalí al centro urbano estilo AoE2). Ciervos/ovejas usan ``flee_on_hit``.
- ``agro_on_sight``: por defecto ``1``. ``0`` impide el fuego de stand ground aunque ``formations 1`` esté activo; solo se contraataca a ``last_attacker`` y los aliados no comparten aggro (jabalí AoE2 DE). Depredadores: ``1`` u omitir.
- ``pursue_leash_range N``: pierde la agresión si está a más de N mm del objetivo (``0`` = ilimitado). Jabalíes usan ``48000`` (~4 casillas); luego olvidan y vuelven a casa.
- ``claimable 1``: mientras es neutro, cualquier unidad no neutral cercana lo reclama (oveja AoE2). Independiente de ``can_herd``.
- Pastizal: ``spawns_unit``, ``larva_spawn_time``, ``larva_cap``, ``spawn_player_cap`` / ``spawn_immediate`` (aoe2 ``pasture`` mongol).
...
Las capas posteriores cubren PNG del mismo nombre. Formato detallado:
``res/ui/anims/README.txt``. Spine es opcional; si falta, se vuelve a spritesheet/iconos/formas.
Código: ``game_unit_anim.py``.

Conjuntos de arquitectura (desde 1.4.8.3): ``ui/architecture.txt`` agrupa civs
(``def <conjunto>`` + ``factions …``). aoe2 usa sets DE, no un miliciano por civ.
Minerales y fauna neutrales quedan en la raíz. RGB (``rim``, etc.) solo para
``python tools/gen_aoe2_hud_icons.py``. Mods con tipos distintos por raza
(StarCraft: ``marine`` / ``zergling`` / ``zealot``) no necesitan subcarpetas.
...
El texto debe existir en ``tts.txt``. Las facciones también pueden definir ``intro``: en el selector pulse **G** para un submenú frase a frase (desde 1.4.7.2).
...
Formaciones (desde 1.5.0.2)
>>>>>>>>>>>>>>>>>>>>>>>>>>>

Desactivadas por defecto. Un mod con ``formations 1`` en ``def parameters`` coloca al grupo militar en huecos dentro de la casilla según ``class formation`` (AoE2 DE: línea, caja, escalonada, flancos).
...
Los nombres de rango son de rules: ``formation_ranks`` los lista (se pueden renombrar o añadir); cada nombre tiene una tabla ``is_a`` ``formation_rank_<nombre>``. Si se omite ``formation_ranks``, se infieren de las claves ``formation_rank_*``. ``formation_default_rank`` si no hay coincidencia; ``formation_range_rank`` cuando ``rdg_range`` es mayor que ``mdg_range``; el frente de combate es el primer rango de ``ranks`` de la formación actual (o ``formation_front_rank``). Los rangos propios usan ``title`` de style; ``melee`` / ``ranged`` / ``siege`` siguen con TTS interno.

``shape``: cartesianas ``line`` / ``box`` / ``staggered`` / ``flank``, o polares ``ring`` / ``arc`` (alias ``circle`` / ``round`` → anillo, ``wedge`` / ``cone`` → arco). Un ``shape`` desconocido con ``radius`` / ``arc_span`` / ``rings`` sigue usando empaquetado polar, sin parchear el motor. ``radius`` es metros (0 = según ``spacing`` y el recuento). ``arc_span`` es grados (0 = 360 en anillo, 180 en arco). ``arc_start`` por defecto es automático. ``rings`` / ``ring_gap`` son concéntricos; ``ring_rank out`` pone el primer rango listado en el exterior. Distancias en metros (PRECISION mm). ``flank_gap`` solo en flancos. ``max_front`` limita la fila (0 = ancho de casilla). Una unidad puede llevar ``use_formation 1`` o ``formation_rank melee``; si no, valen las listas ``is_a``. El menú lista ``set_formation <tipo>`` por cada ``class formation`` (elegir el actual reordena en cada casilla; seleccionar todo el ejército no junta casillas lejanas). ``cycle_formation`` no entra en el menú hablado; tecla por defecto ``CTRL SHIFT f``, cambiable en el editor. Style: ``def set_formation`` y el ``title`` de cada formación; ``parameters.formation_change`` es el SFX. El ``go`` de grupo a una casilla conserva filas; ``go`` / ``attack`` sobre una unidad rompe huecos y se concentra. El ``offensive`` ocioso al contactar sigue en formación. Un enemigo formado en el camino intercepta (``formation_blocker``). Con formaciones, ``guard`` es mantener posición: dispara a rango, no camina (salvo ``agro_on_sight 0``, jabalí AoE2 DE). ``chase`` sigue cerrando. ``class formation`` puede añadir ``mdg`` / ``rdg`` / ``mdf`` / ``rdf`` y ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs`` solo al mantener huecos (se pierde con ``break_formation_hold`` / fuego concentrado). ``speed`` (puede ser negativo, p. ej. ``speed -1.5`` o ``speed -30%``) se suma después del tope ``keep_pace``. Valores absolutos o porcentajes de la stat de esa unidad (``mdg 20%``). Por defecto 0; los cuatro tipos de AoE2 siguen siendo solo disposición. Con filas mantenidas, ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs`` también coinciden con el nombre ``class formation`` actual del otro (p. ej. ``formation_wedge``) y la forma (``cone`` / ``wedge`` / ``arc``, …), sumados al vs por tipo de unidad; si coinciden varias claves de forma, solo vale la más específica. Se escriben identificadores, no títulos traducidos. Sin bonus si el otro ha roto filas, persigue o aun no ha llegado al hueco (`formation_ranks_formed`). `keep_pace` sigue al caminar.
