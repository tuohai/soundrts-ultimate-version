...
**Cambio: ningún ataque automático de la IA durante el tratado en ningún modo**

- **Problema**: ``AttackOrder.execute`` ya tenía una puerta de tratado, pero las decisiones de la IA (``_attack()``) y ``AttackAction.update()`` en curso aún podían emitir/continuar ataques contra unidades enemigas. Una vez arreglado el modo guardia, al cambiar aldeanos a modo persecución/defensivo/ataque podían auto-atacar y matar enemigos durante el tratado.
- **Cambio**: Se añadió una comprobación de ``treaty_until_time`` al inicio tanto de ``AttackActionMixin._attack()`` como de ``AttackAction.update()``. Las unidades enemigas de jugadores se bloquean (con exención para ``is_wildlife_unit``) y los ``AttackAction`` en curso contra enemigos se completan. Todos los modos de IA (guardia/persecución/defensivo/ataque) ya no pueden atacar automáticamente a enemigos durante el tratado.
- **Alcance**: ``soundrts/combat/attack_action.py``; ``soundrts/worldaction.py``.

**Cambio: el modo defensivo no se retira durante el tratado**

- **Problema**: El modo defensivo huía con una orden ``go`` cuando el balance era malo. Durante el tratado, que un enemigo se acerque no debería hacer que nuestras unidades se retiren automáticamente; eso no encaja con el diseño del modo Treaty de AoE2 DE.
- **Cambio**: Se añadió una exención de tratado a la condición de retirada en modo defensivo en ``decide()``. Ninguna unidad de la IA puede huir de una amenaza mientras ``treaty_until_time`` esté vigente.
- **Alcance**: ``soundrts/worldunit/world_ai_decision.py``.
...
**Cambio: el alcance del campana de pueblo es el edificio pulsado**

- **Problema**: Tocar cualquier edificio ``town_bell`` unía el ``town_bell_range`` de todas las campanas. Tocar a1 metía en b3 a aldeanos que solo estaban en las dos casillas de b3.
- **Cambio**: ``ring_town_bell`` mide ``town_bell_range`` solo desde el edificio pulsado (AoE2 sigue usando ``24`` metros como equivalente local de unas 25 casillas). Los trabajadores en ese radio entran en el edificio guarnecible más cercano con sitio. ``workers_to_garrison`` exige ese edificio.
- **Alcance**: ``world_town_bell.py``; ``worldorders/immediate.py``; ``test_town_bell.py``; ``test_changelog_15.py``.

1.5.0.5
-------

**Cambio: recorte del camino caliente sin ventana**

- **Problema**: Con muchos IA, ``known_enemies`` recorría todas las unidades interiores y llamaba ``container_visible_from_place`` por casilla; el cebo de jabalí, agua/anfibio y ``can_reach`` se recalculaban cada turno; ``update`` reimportaba ``VIRTUAL_TIME_INTERVAL``.
- **Cambio**: ``index_inside_units_visible_from_places`` indexa por casilla. ``known_enemies`` usa ``_enemy_inside_index``. El cebo, transportes de agua/aire y ``can_reach`` usan ``_play_memo_get``. Mapas terrestres omiten obreros de agua y desembarcos anfibios. ``menace_versus`` usa ``getattr``. ``VIRTUAL_TIME_INTERVAL`` se importa a nivel de módulo.
...
**Cambio: AoE2 y las reglas base comentan el space de las unidades**

- **Problema**: El ``space`` de unidad ocupaba capacidad abstracta por alianza, y un ejército grande no cabía en una casilla.
- **Cambio**: Las líneas ``space`` de ``mods/aoe2/rules.txt`` y ``res/rules.txt`` quedan como ``;space``. Si se omite, ``space`` sigue en ``0`` (sin ocupación abstracta). La colisión física y las ranuras de formación no cambian. Descomente para volver al límite de capacidad.
- **Alcance**: ``mods/aoe2/rules.txt``; ``res/rules.txt``; ``test_changelog_15.py``.

**Cambio: las invocaciones que caducan no cuentan como bajas**

- **Problema**: Al vencer ``time_limit``, los efectos llamaban ``die()`` y sumaban ``lost``. Las invocaciones normales usaban ``on_disappear``, pero ``lang_add_units`` aún las metía en ``produced``, así que supervivencia y producción enemiga se torcían con tropas temporales.
- **Cambio**: El caducar ``on_disappear`` no suma ``lost``; ``uncount_expired_unit_produced`` revierte ``produced``. El tiempo de los efectos también usa ``on_disappear``. Morir en combate sigue contando ``lost`` / ``killed``.
- **Alcance**: ``worldplayerstats.py``; ``worldunit/world_status_update.py``; ``worldunit/worldeffect.py``; ``worldunit/world_attributes.py``; ``worldunit/worldcreature.py``; ``test_score_breakdown.py``; ``test_changelog_15.py``; ``score-grading-system.rst`` y ``score-and-grades.rst`` en las cinco lenguas.

1.5.0.4
-------

**Cambio: los campos de acierto pasan a mdg_hit_rate / rdg_hit_rate**

- **Problema**: el acierto en rules era ``mdg_cover`` / ``rdg_cover`` (0 = 100%), fácil de confundir con el ocultamiento de terreno ``cover`` / ``cover_vs``.
- **Cambio**: Pasan a ``mdg_hit_rate`` / ``rdg_hit_rate``, más ``*_hit_rate_vs``, ``*_hit_rate_on_terrain``, ``*_hit_rate_bonus``. Sin alias ``*_cover``. El ``cover`` / ``cover_vs`` de terreno no cambia. Los ids TTS siguen ``MDG_COVER``; el ``msgid`` de gettext pasa a ``mdg_hit_rate_vs`` / ``*_hit_rate_on_terrain``.
- **Alcance**: ``combat/hit_miss.py``; ``definitions.py``; ``worldunit``; ``mods/aoe2/rules.txt``; ``i18n/tts.pot``; ``tts-*.po``; ``test_unit_vs_params_runtime.py``; ``test_changelog_15.py``; ``modding.rst`` en los cinco idiomas.

**Cambio: los campos de esquiva pasan a mdg_dodge_rate / rdg_dodge_rate**

- **Problema**: la esquiva en rules era ``mdg_dodge`` / ``rdg_dodge``, fácil de confundir con los sonidos de esquiva de style ``mdg_dodge`` / ``rdg_dodge`` y el ``dodge_vs`` de terreno.
- **Cambio**: Pasan a ``mdg_dodge_rate`` / ``rdg_dodge_rate``, más ``*_dodge_rate_vs``, ``*_dodge_rate_on_terrain``, ``*_dodge_rate_bonus``. Sin alias de atributo. El ``dodge_vs`` de terreno y los sonidos de esquiva de style no cambian. Los ids TTS siguen ``MDG_DODGE``; el ``msgid`` de gettext pasa a ``mdg_dodge_rate_vs`` / ``*_dodge_rate_on_terrain``.
- **Alcance**: ``combat/hit_miss.py``; ``definitions.py``; ``worldunit``; ``worldweapon.py``; ``i18n/tts.pot``; ``tts-*.po``; ``test_unit_vs_params_runtime.py``; ``test_changelog_15.py``; ``modding.rst`` en los cinco idiomas.

**Cambio: los rangos de formación son totalmente de rules**

- **Problema**: los nombres de rango estaban fijos como ``melee`` / ``ranged`` / ``siege``. La búsqueda ``formation_rank_*``, el respaldo por alcance, el frente de combate y las etiquetas de voz no se podían renombrar ni añadir un cuarto rango.
- **Cambio**: ``def parameters`` lista los rangos en ``formation_ranks`` (si se omite, se infieren de las claves ``formation_rank_<nombre>``). Cada nombre tiene una tabla ``is_a`` ``formation_rank_<nombre>``. Opcionales ``formation_default_rank``, ``formation_range_rank`` (cuando ``rdg_range`` es mayor que ``mdg_range``) y ``formation_front_rank``. El frente de combate usa el primer rango de ``ranks`` de la formación actual. Un ``class formation`` sin ``ranks`` usa ``formation_ranks``. Los nombres propios usan ``title`` de style; ``melee`` / ``ranged`` / ``siege`` siguen con TTS interno. El mod AoE2 escribe ``formation_ranks melee ranged siege`` de forma explícita.
- **Alcance**: ``world_formation.py``; ``definitions.py``; ``mods/aoe2/rules.txt``; ``test_world_formation.py``; ``test_changelog_15.py``; ``modding.rst`` en los cinco idiomas.
...
**Cambio: la duración del tratado coincide con Age of Empires II DE, con opción personalizada**

- **Problema**: el tratado previo solo ofrecía 0/5/10/15/20 minutos; 20 era corto para partidas lentas, y valores como 12 o 75 había que redondearlos al peldaño más cercano.
- **Cambio**: las opciones coinciden con AoE2 DE: ``TREATY_MINUTE_CHOICES`` son pasos de 5 minutos hasta 60, más 90. Bajo las predefinidas, «tratado personalizado» acepta un entero de 5 a 90 (``parse_custom_treaty_minutes``); fuera de rango o no entero suena un aviso y vuelve al menú de tratado, y Esc cancela igual. El protocolo sigue pasando minutos a milisegundos; valores por encima de 90 se recortan a 90.
- **Alcance**: ``treaty.py``; ``clientservermenu.py``; ``clientmain.py``; ``randommap_menu.py``; ``game.py``; ``serverroom.py``; ``msgparts.py`` ``ENTER_TREATY_MINUTES``; ``test_changelog_1422_treaty_coop.py``; ``test_changelog_15.py``.

**Cambio: formaciones gobernadas por reglas (class formation genérico)**

- **Problema**: un grupo militar seleccionado se movía como unidades sueltas. No había línea / caja / escalonada / flancos, ni filas de cuerpo a cuerpo, a distancia y asedio. Una geometría nueva clavada en el motor obligaría a parchearlo por cada tipo definido por el jugador.
- **Cambio**: se activa con ``formations 1`` en ``def parameters``. El menú lista cada ``class formation`` de las reglas (``formation_line`` / ``formation_box`` / ``formation_staggered`` / ``formation_flank`` del mod AoE2 son ejemplos que un mod puede cambiar o ampliar). ``shape`` es cartesiano line / box / staggered / flank, o polar ``ring`` / ``arc`` (alias ``circle`` / ``round``, ``wedge`` / ``cone``). Un nombre desconocido con ``radius`` / ``arc_span`` / ``rings`` sigue usando empaquetado polar, sin parchear el motor. Distancias (``spacing``, ``rank_gap``, ``flank_gap``, ``radius``) en metros; ``arc_span`` en grados. ``ranks`` es el orden de filas. ``formation_units`` y ``formation_rank_melee`` / ``formation_rank_ranged`` / ``formation_rank_siege`` usan ``is_a``; una unidad también puede llevar ``use_formation 1`` o ``formation_rank``. El ``go`` de grupo escribe un ``ZoomTarget`` por unidad en la casilla de destino, mirando al movimiento; ``keep_pace`` iguala al más lento. El menú solo lista los tipos con nombre de ``set_formation`` (elegir el actual reordena en el sitio). ``cycle_formation`` no entra en el menú hablado y se puede asignar (por defecto ``CTRL SHIFT f``). El mod AoE2 usa línea por defecto.
- **Alcance**: ``world_formation.py``; ``worldorders/immediate.py``; ``definitions.py``; ``mods/aoe2/rules.txt``; ``hotkey_catalogs.py``; ``test_world_formation.py``.

**Cambio: mantener filas al estilo Age of Empires II al combatir**

- **Problema**: las formaciones solo colocaban huecos en el ``go``. Las unidades ``offensive`` luego corrían cada una un ``AttackAction`` al objetivo, así que los arqueros de atrás entraban a cuerpo a cuerpo y una línea luchaba como un montón. ``keep_pace`` ponía ``_formation_speed_cap`` pero el andar usaba ``actual_speed`` sin tope, así que la caballería llegaba primero.
- **Cambio**: un ``attack`` de grupo (o un movimiento sobre un enemigo) mira la amenaza con cuerpo a cuerpo delante y a distancia/asedio detrás (``apply_combat_formation``). Quien está a rango dispara en el sitio; quien no, anda a su hueco en vez de amontonarse en un objetivo. ``chase`` sigue cerrando. ``actual_speed`` / ``current_speed`` aplican el tope al leerse.
- **Alcance**: ``world_formation.py``; ``worldunit/world_movement.py``; ``worldunit/world_attributes.py``; ``combat/attack_action.py``; ``worldorders/movement.py``; ``test_formation_combat.py``; ``test_world_formation.py``.

**Cambio: formaciones más cerca del bloqueo, fuego concentrado y mantener posición de AoE2**

- **Problema**: las filas no tenían muro de colisión, así que las unidades atravesaban la línea enemiga hasta la fila de atrás. Clicar un enemigo concreto mantenía huecos en vez de amontonarse. Con formaciones, ``guard`` ni disparaba a rango ni se quedaba plantado: la formación de combate seguía empujando.
- **Cambio**: un enemigo formado en el segmento de marcha es la intercepción (``formation_blocker``). El ``go`` de grupo a una casilla conserva filas; ``go`` / ``attack`` sobre una unidad llama ``break_formation_hold`` y se concentra. El ``offensive`` ocioso al contactar sigue en filas. ``guard`` con ``formations 1`` es mantener posición: dispara si ``_near_enough_to_aim``, nunca anda.
- **Alcance**: ``world_formation.py``; ``worldunit/world_movement.py``; ``worldunit/world_ai_decision.py``; ``worldorders/movement.py``; ``test_world_formation.py``; ``test_formation_combat.py``.

**Cambio: cambiar de formación con todo el ejército seleccionado reordena en cada casilla**

- **Problema**: tras Ctrl+S, elegir línea (u otro tipo) ejecutaba ``apply_idle_rearrange`` alrededor del centroide del ejército, así que unidades en casillas lejanas andaban a una sola casilla.
- **Cambio**: al reordenar unidades ociosas se agrupa por la casilla en la que están (``_units_grouped_by_place``). Cada casilla se alinea en el sitio; las demás no se mueven. Las unidades ya en ``go`` se agrupan por destino, así que marchas a casillas distintas no se fusionan.
- **Alcance**: ``world_formation.py``; ``test_world_formation.py``.

**Cambio: class formation puede definir stats de combate y speed (absolutos o porcentajes)**

- **Problema**: las formaciones solo colocaban huecos, sin canje de daño/armadura cuerpo a cuerpo o a distancia ni de velocidad. ``mdg 2`` / ``speed -1.5`` absolutos dan los mismos puntos a infantería y caballería.
- **Cambio**: ``class formation`` puede definir ``mdg`` / ``rdg`` / ``mdf`` / ``rdf`` y ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs``, más ``speed`` (puede ser negativo). Valores absolutos como ``mdg 2`` y ``speed -1.5``, o porcentajes como ``mdg 20%`` y ``speed -30%`` de la stat de esa unidad. Solo al mantener huecos (``speed`` después del tope ``keep_pace``, en ``actual_speed``); se pierde con ``break_formation_hold`` / fuego concentrado. Por defecto 0.
- **Alcance**: ``world_formation.py``; ``combat/damage_calculation.py``; ``worldunit/world_attributes.py``; ``test_world_formation.py``.

**Cambio: Alt+V muestra tipos de formación y efectos**

- **Problema**: la pantalla de propiedades no listaba la formación actual, los tipos disponibles ni la forma y los efectos de combate/velocidad de ``class formation``.
- **Cambio**: las unidades que pueden formar muestran la formación actual, las formaciones disponibles (izquierda/derecha, Intro para detalles) y la fila de esa unidad. El detalle lista forma, separación, mantener el paso, y bonificaciones como ``mdg`` / ``speed`` (absolutas o porcentajes).
- **Alcance**: ``formation_detail.py``; ``AVAILABLE_FORMATIONS``; ``class formation``; ``test_formation_attributes_ui.py``.

**Cambio: las bonificaciones de HP por edad también suben la vida actual**

- **Problema**: el ``on_phase feudal_age hp_max 20% cavalry`` franco solo subía el techo. Un explorador 45/45 pasaba a 45/54 al Feudal; la caballería nueva salía igual.
- **Cambio**: las bonificaciones de HP de civ usan ``hp 20% cavalry`` (como Bloodlines ``effect bonus hp 20`` y Gran Muralla ``hp 30%``): máximo y actual crecen el mismo delta. Explorador lleno 45/45 → 54/54. Igual para exploradores mongoles, infantería vikinga, arqueros vietnamitas, barcos portugueses y barcos de pesca japoneses.
- **Alcance**: ``mods/aoe2/rules.txt``; ``test_hp_max_bonus_current_hp.py``.
...
**Cambio: en mods multirraciales la IA ya no persigue el ayuntamiento de otra raza**

- **Problema**: las obreras insecto de CrazyMod (``ouvriere_marcheuse``) no tienen ``can_build`` (los edificios salen de habilidades de la incubadora). La IA unía las listas de todos los aldeanos, iba a por ``elevage`` / ``mairie`` técnicos y registraba ``AI has trouble getting: 1 ['point_de_depart', 'townhall', 'travailleur']``.
- **Cambio**: con trabajadores vivos solo cuenta su ``can_build`` (aunque esté vacío). Sin trabajadores, solo el ``peasant`` de la tabla de facción; no se recorren todas las razas.
- **Alcance**: ``worldplayercomputer.py`` ``_worker_buildable_type_names`` / ``_faction_peasant_type_name``; ``test_crazymod_pra1_ai.py``.
...
**Corrección: cambiar la velocidad en Opciones no se aplicaba hasta reiniciar**

- **Problema**: ``Game.run(speed=config.speed)`` congelaba el valor al importar. Pasar de personalizado 6 a 1 en Opciones escribía ``SoundRTS.ini``, pero solo/campaña ``run()`` seguía usando el 6 del arranque.
- **Cambio**: leer ``current_game_speed()`` al iniciar la partida.
