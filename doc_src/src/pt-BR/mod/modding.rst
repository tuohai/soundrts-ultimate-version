...
- Atributos de animal: ``is_huntable``, ``flee_on_hit``, ``agro_on_sight``, ``pursue_attacker``, ``herdable``, ``food_deposit``, ``food_deposit_qty``, ``no_number``.
...
- ``agro_on_sight``: omissão ``1``. ``0`` impede fogo de stand ground mesmo com ``formations 1``; só contra-ataca ``last_attacker`` e aliados não partilham aggro (javali AoE2 DE). Predadores: ``1`` ou omitir.
