...
**Mudança: os campos de acerto passam a mdg_hit_rate / rdg_hit_rate**

- **Problema**: o acerto nas rules era ``mdg_cover`` / ``rdg_cover`` (0 = 100%), fácil de confundir com o ocultamento de terreno ``cover`` / ``cover_vs``.
- **Mudança**: Passam a ``mdg_hit_rate`` / ``rdg_hit_rate``, mais ``*_hit_rate_vs``, ``*_hit_rate_on_terrain``, ``*_hit_rate_bonus``. Sem aliases ``*_cover``. O ``cover`` / ``cover_vs`` de terreno não muda. Os ids TTS continuam ``MDG_COVER``; o ``msgid`` gettext passa a ``mdg_hit_rate_vs`` / ``*_hit_rate_on_terrain``.
- **Alcance**: ``combat/hit_miss.py``; ``definitions.py``; ``worldunit``; ``mods/aoe2/rules.txt``; ``i18n/tts.pot``; ``tts-*.po``; ``test_unit_vs_params_runtime.py``; ``test_changelog_15.py``; ``modding.rst`` nas cinco línguas.

**Mudança: os campos de esquiva passam a mdg_dodge_rate / rdg_dodge_rate**

- **Problema**: a esquiva nas rules era ``mdg_dodge`` / ``rdg_dodge``, fácil de confundir com os sons de esquiva de style ``mdg_dodge`` / ``rdg_dodge`` e o ``dodge_vs`` de terreno.
- **Mudança**: Passam a ``mdg_dodge_rate`` / ``rdg_dodge_rate``, mais ``*_dodge_rate_vs``, ``*_dodge_rate_on_terrain``, ``*_dodge_rate_bonus``. Sem aliases de atributo. O ``dodge_vs`` de terreno e os sons de esquiva de style não mudam. Os ids TTS continuam ``MDG_DODGE``; o ``msgid`` gettext passa a ``mdg_dodge_rate_vs`` / ``*_dodge_rate_on_terrain``.
- **Alcance**: ``combat/hit_miss.py``; ``definitions.py``; ``worldunit``; ``worldweapon.py``; ``i18n/tts.pot``; ``tts-*.po``; ``test_unit_vs_params_runtime.py``; ``test_changelog_15.py``; ``modding.rst`` nas cinco línguas.

**Mudança: as filas de formação são totalmente definidas nas rules**

- **Problema**: os nomes das filas estavam fixos em ``melee`` / ``ranged`` / ``siege``. A procura ``formation_rank_*``, o recurso por alcance, a frente de combate e as etiquetas de voz não se podiam mudar de nome nem acrescentar uma quarta fila.
- **Mudança**: ``def parameters`` lista as filas em ``formation_ranks`` (se omitido, os nomes inferem-se das chaves ``formation_rank_<nome>``). Cada nome tem uma tabela ``is_a`` ``formation_rank_<nome>``. Opcionais ``formation_default_rank``, ``formation_range_rank`` (quando ``rdg_range`` é maior que ``mdg_range``) e ``formation_front_rank``. A frente de combate usa a primeira fila de ``ranks`` da formação atual. Um ``class formation`` sem ``ranks`` usa ``formation_ranks``. Nomes próprios usam ``title`` de style; ``melee`` / ``ranged`` / ``siege`` mantêm TTS interno. O mod AoE2 escreve explicitamente ``formation_ranks melee ranged siege``.
- **Alcance**: ``world_formation.py``; ``definitions.py``; ``mods/aoe2/rules.txt``; ``test_world_formation.py``; ``test_changelog_15.py``; ``modding.rst`` nas cinco línguas.
...
**Mudança: a duração do tratado alinha Age of Empires II DE, com opção personalizada**

- **Problema**: o tratado pré-jogo só oferecia 0/5/10/15/20 minutos; 20 era curto demais para partidas lentas, e valores como 12 ou 75 obrigavam a escolher o degrau mais próximo.
- **Mudança**: as opções coincidem com AoE2 DE: ``TREATY_MINUTE_CHOICES`` são passos de 5 minutos até 60, mais 90. Abaixo das predefinições, «tratado personalizado» aceita um inteiro de 5 a 90 (``parse_custom_treaty_minutes``); fora do intervalo ou não inteiro: bipe e volta ao menu do tratado; Esc cancela da mesma forma. O protocolo ainda converte minutos em milissegundos; valores acima de 90 são limitados a 90.
...
**Mudança: formações guiadas por regras (class formation genérico)**

- **Problema**: um grupo militar selecionado andava como unidades soltas. Não havia linha / caixa / escalonada / flancos, nem filas de corpo a corpo, à distância e de cerco. Uma geometria nova cravada no motor obrigaria a um patch por cada tipo definido pelo jogador.
- **Mudança**: liga-se com ``formations 1`` em ``def parameters``. O menu lista cada ``class formation`` das regras (``formation_line`` / ``formation_box`` / ``formation_staggered`` / ``formation_flank`` do mod AoE2 são exemplos que um mod pode alterar ou alargar). ``shape`` é cartesiano line / box / staggered / flank, ou polar ``ring`` / ``arc`` (aliases ``circle`` / ``round``, ``wedge`` / ``cone``). Um nome desconhecido com ``radius`` / ``arc_span`` / ``rings`` continua a usar empacotamento polar — sem alterar o motor. Distâncias (``spacing``, ``rank_gap``, ``flank_gap``, ``radius``) em metros; ``arc_span`` em graus. ``ranks`` é a ordem das filas. ``formation_units`` e ``formation_rank_melee`` / ``formation_rank_ranged`` / ``formation_rank_siege`` usam ``is_a``; uma unidade também pode ter ``use_formation 1`` ou ``formation_rank``. O ``go`` do grupo grava um ``ZoomTarget`` por unidade no quadrado de destino, virado para o movimento; ``keep_pace`` segue a mais lenta. O menu lista só os tipos nomeados de ``set_formation`` (escolher o atual reorganiza no sítio). ``cycle_formation`` fica fora do menu falado e pode ser atribuída (predefinição ``CTRL SHIFT f``). O mod AoE2 usa linha por omissão.
...
**Mudança: manter filas ao estilo Age of Empires II em combate**

- **Problema**: as formações só punham slots no ``go``. Unidades ``offensive`` depois cada uma corria um ``AttackAction`` ao alvo, por isso os arqueiros da fila de trás iam ao corpo a corpo e uma linha lutava como um monte. ``keep_pace`` punha ``_formation_speed_cap`` mas o andar usava ``actual_speed`` sem teto, por isso a cavalaria chegava primeiro.
- **Mudança**: um ``attack`` de grupo (ou um movimento sobre um inimigo) enfrenta a ameaça com corpo a corpo à frente e à distância/cerco atrás (``apply_combat_formation``). Quem está ao alcance dispara no sítio; quem não está anda ao próprio slot em vez de amontoar-se num alvo. ``chase`` continua a fechar. ``actual_speed`` / ``current_speed`` aplicam o teto à leitura.
...
**Mudança: formações mais perto do bloqueio, fogo concentrado e manter posição de AoE2**

- **Problema**: as filas não tinham parede de colisão, por isso as unidades atravessavam a linha inimiga até à fila de trás. Clicar um inimigo concreto mantinha slots em vez de concentrar. Com formações, ``guard`` nem disparava ao alcance nem ficava plantado — a formação de combate ainda empurrava.
- **Mudança**: um inimigo em formação no segmento de marcha é a interceção (``formation_blocker``). O ``go`` de grupo para um quadrado mantém filas; ``go`` / ``attack`` numa unidade chama ``break_formation_hold`` e concentra. O ``offensive`` ocioso ao contactar mantém as filas. ``guard`` com ``formations 1`` é manter posição: dispara se ``_near_enough_to_aim``, nunca anda.
...
**Mudança: mudar de formação com todo o exército selecionado reorganiza em cada quadrado**

- **Problema**: depois de Ctrl+S, escolher linha (ou outro tipo) corria ``apply_idle_rearrange`` em torno do centróide do exército, por isso unidades em quadrados distantes andavam para um só quadrado.
- **Mudança**: ao reorganizar, unidades ociosas agrupam-se pelo quadrado em que estão (``_units_grouped_by_place``). Cada quadrado alinha no sítio; os outros ficam. Unidades já em ``go`` agrupam-se pelo destino, por isso marchas para quadrados diferentes não se fundem.
- **Alcance**: ``world_formation.py``; ``test_world_formation.py``.

**Mudança: class formation pode definir stats de combate e speed (absolutos ou percentagens)**

- **Problema**: as formações eram só disposição, sem troca de dano/armadura corpo a corpo ou à distância nem de velocidade. ``mdg 2`` / ``speed -1.5`` absolutos dão os mesmos pontos a infantaria e cavalaria.
- **Mudança**: ``class formation`` pode definir ``mdg`` / ``rdg`` / ``mdf`` / ``rdf`` e ``mdg_vs`` / ``rdg_vs`` / ``mdf_vs`` / ``rdf_vs``, mais ``speed`` (pode ser negativo). Valores absolutos como ``mdg 2`` e ``speed -1.5``, ou percentagens como ``mdg 20%`` e ``speed -30%`` da stat dessa unidade. Só ao manter slots (``speed`` depois do teto ``keep_pace``, em ``actual_speed``); perdem-se com ``break_formation_hold`` / fogo concentrado. Por omissão 0.
- **Alcance**: ``world_formation.py``; ``combat/damage_calculation.py``; ``worldunit/world_attributes.py``; ``test_world_formation.py``.

**Mudança: Alt+V mostra tipos de formação e efeitos**

- **Problema**: a tela de propriedades não listava a formação atual, os tipos disponíveis nem a forma e os efeitos de combate/velocidade de ``class formation``.
- **Mudança**: unidades que podem formar mostram a formação atual, as formações disponíveis (esquerda/direita, Enter para detalhes) e a fileira da unidade. O detalhe lista forma, espaçamento, manter o passo, e bónus como ``mdg`` / ``speed`` (absolutos ou percentagens).
- **Alcance**: ``formation_detail.py``; ``AVAILABLE_FORMATIONS``; ``class formation``; ``test_formation_attributes_ui.py``.

**Mudança: bónus de HP por idade também sobem a vida atual**

- **Problema**: ``on_phase feudal_age hp_max 20% cavalry`` dos Francos só subia o teto. Um batedor 45/45 ficava 45/54 no Feudal; cavalaria nova nascia igual.
- **Mudança**: bónus de HP de civ usam ``hp 20% cavalry`` (como Bloodlines ``effect bonus hp 20`` e Grande Muralha ``hp 30%``): máximo e atual crescem o mesmo delta. Batedor cheio 45/45 → 54/54. Igual para batedores mongóis, infantaria viking, arqueiros vietnamitas, navios portugueses e barcos de pesca japoneses.
...
- **Problema**: ``attack_warning_wild_animal`` estava no ``alert`` de veado / ovelha / javali. O motor toca o ``alert`` da **vítima** quando a tua unidade é ferida, por isso um javali a morder um aldeão usava ``attack_warning_general``.
- **Mudança**: o style da vítima pode definir ``alert_<tipo atacante>``, percorrendo o ``is_a`` do atacante (do mais específico). AoE2: ``def animal``, javali / veado / ovelha ``is_a animal``, camponês ``alert_animal attack_warning_wild_animal``; ``alert_boar`` / ``alert_wolf`` também servem. Aldeões de civ ``is_a peasant`` herdam. Edifícios continuam com ``alert`` (``attack_warning_base``). O intervalo de 10 s não muda.
- **Alcance**: ``clientgameentity/combat.py``; ``mods/aoe2/ui/style.txt``; ``test_wild_attack_alert.py``.

**Mudança: sinalizador com tecla, fala e nome / som por regras**

- **Problema**: depois do overlay AoE2, ``CTRL+SHIFT+N`` não fazia nada e os esquemas não tinham ``flare``. Só ``parameters.signal_flare`` panorâmico não dizia o quadrado. Nome e som estavam fixos, por isso outros mods não podiam desligar nem mudá-los.
- **Mudança**: ativa-se com ``signal_flare 1`` em ``rules.txt`` ``def parameters``. No style, ``parameters.signal_flare`` é o efeito; ``signal_flare_title`` é o nome (TTS ``SIGNAL_FLARE`` / 5842 por omissão). Aliados ouvem «sinalizador em <quadrado>» (se outro enviar, o nome vem primeiro). AoE2 em camadas / clássico / RPG liga ``CTRL SHIFT n: flare``; no mapa continua ``N``. Os esquemas só o listam se estiver ativo.
...
**Problema: unidades sem mochila ouviam «mochila vazia» / «equipamento vazio»**

- **Problema**: sem ``inventory_capacity`` não há mochila nem barra de equipamento. ``F3`` em camadas ou ``Shift+V`` clássico mesmo assim dizia «mochila vazia».
- **Mudança**: se ``inventory_capacity`` for 0 (predefinição se omitido), só um beep; não abre ecrãs nem diz ``EMPTY_BACKPACK`` / equipamento vazio. Com capacidade e saco vazio esses avisos continuam.
...
**Problema: ao avançar a idade não tocava age_advance**

- **Problema**: o pacote AoE2 tinha ``age_advance.mp3`` em ``ui/music/``. Avançar a idade usa a cache SFX de ``upgrade_complete``, que antes só indexava ``.ogg`` em ``ui/`` e não tratava ``ui/music/`` como SFX curtos, por isso a fanfarra ficava muda.
- **Mudança**: a cache SFX indexa ``.ogg`` / ``.wav`` (``.ogg`` ganha no mesmo stem) e continua a ignorar ``ui/music/`` (BGM em ciclo, mp3 permitido). SFX curtos ficam em ``ui/sounds/``. ``age_advance`` e ``wolf`` estão em ``.ogg``. ``upgrade_complete age_advance`` do centro da cidade não muda.
- **Alcance**: ``sound_cache.py``; ``mods/aoe2/ui/sounds/age_advance.ogg``; ``test_ui_sfx_stem.py``; ``test_changelog_1499.py``.

1.4.9.8
--------

**Mudança: sino da cidade guiado por regras**

- **Problema**: os centros da cidade AoE2 podem meter aldeões próximos em edifícios e depois devolvê-los ao trabalho. O motor não tinha essa ordem, por isso ``town_bell1`` / ``town_bell2`` não tinham onde tocar.
- **Mudança**: edifícios com ``town_bell 1`` ganham tocar / voltar ao trabalho. O alcance é em metros euclidianos (``town_bell_range``, mm PRECISION; ``0`` = mapa inteiro), não BFS por casas. Por omissão chama trabalhadores de terra (não barcos); ``town_bell_units`` filtra tipos. O primeiro toque mete os próximos no edifício guarnecível mais próximo com espaço e guarda as ordens; aldeões já dentro também são marcados. O segundo tira esses aldeões e restaura colheita/construção (soldados guarnecidos à mão ficam). Depois de meter aldeões no centro da cidade ainda podes tocar; Voltar ao trabalho tira-os. Os centros AoE2 usam ``24`` metros (cerca de duas casas). SFX: ``town_bell1`` / ``town_bell2``.
...
**Mudança: SFX de derrota alheia, diplomacia, objetivos, sinalizador, ponto de reunião e população cheia**

- **Problema**: ``playerdefeated``, ``diplomacy_change``, ``objective_change``, ``signal_flare``, ``gather_point`` e ``population_limit`` não tinham eventos. População cheia ainda usava ``error``.
- **Mudança**: se outro jogador desistir ou for eliminado, toca ``parameters.player_defeated`` (a tua vitória/derrota continua ``victory`` / ``defeat``). Formar ou romper uma aliança toca ``diplomacy_change``. Acrescentar, completar ou abandonar um objetivo toca ``objective_change``. ``CTRL+SHIFT+N`` na casa atual (``N`` ao explorar o mapa) avisa os aliados. Definir o ponto de reunião toca ``rallying_point`` (AoE2: ``gather_point``). População cheia toca ``population_limit``. Cada mod mapeia-os em ``parameters`` do ``style``; não chocam com o pacote res.
...
**Mudança: em mods multi-raça o computador já não persegue o centro da cidade de outra raça**

- **Problema**: as operárias inseto do CrazyMod (``ouvriere_marcheuse``) não têm ``can_build`` (os edifícios vêm de habilidades da chocadeira). O computador unia as listas de todos os aldeões, ia atrás de ``elevage`` / ``mairie`` técnicos e registava ``AI has trouble getting: 1 ['point_de_depart', 'townhall', 'travailleur']``.
- **Mudança**: com trabalhadores vivos conta só o ``can_build`` deles (mesmo vazio). Sem trabalhadores, só o ``peasant`` da tabela de facção; não se percorrem todas as raças.
