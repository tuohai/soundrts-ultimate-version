...
| Term | Meaning |
| --- | --- |
| **Host** | Barracks, Factory, or Starport (`can_have_addon`) |
| **Addon** | Tech Lab or Reactor (`is_addon 1`), built on the host’s **side slot** |
| **Slot** | About **3.5 tiles** east of the host (`addon_offset_x`, default 3500 internal units) |
| **Meadow** | A `meadow` object on the square; ground Terran buildings must **consume** one to land |
| **Recombine** | After lift-off the addon stays on the ground; another host lands with the **slot aligned** and auto-reattaches the orphan addon |
...
| Step | Decides | Does not decide |
| --- | --- | --- |
| **Landing** | Which meadow is consumed; host **(x, y)** | Reattach |
| **Reattach** | Orphan Tech Lab attaches to host | Which meadow was used |
...
| Goal | Tab | After go | change_to result |
| --- | --- | --- | --- |
| Land on lift spot | Lift-off meadow | Backspace | No reattach, no Tank |
| Take orphan Tech Lab | **Tech Lab** | Backspace (flies to slot) | Reattach, Tank available |
...
| Keyword | Role |
| --- | --- |
| `can_have_addon` | Allowed addon types on host |
| `is_addon 1` | Addon building |
| `addon_host_types` | Which hosts accept this addon |
| `addon_grants_train_<host>` | Extra train options when attached |
| `addon_grants_research` | Extra research when attached |
| `addon_train_multiplier` | Reactor multiplier |
| `can_change_to` / `ground_form` | Lift / land forms |
| `change_time` | Morph time |

See also [`mods/starcraft/readme.txt`](../../../mods/starcraft/readme.txt).  
Author reference: [`doc_src/src/en/modding.rst`](../../../doc_src/src/en/modding.rst) section *Build fields, addons & lift-off*.
