...
> **Full guide (1.4.3.9):** [../player/campaign-and-co-op-improvements.md](../player/campaign-and-co-op-improvements.md) — mission browser, difficulty tiers, AI partners, determinism, map authoring.  
> 中文：[../../zh/player/战役与合作战役改进说明.md](../../zh/player/战役与合作战役改进说明.md)
...
Like Age of Empires campaign tables, co-op is declared in **`campaign.txt`**
...
| Field | Meaning |
|-------|---------|
| `coop_campaign` | `1` — campaign appears in the server **Co-op campaign** menu |
| `coop_intro` | Cutscene chapter numbers shown in the co-op flow (e.g. prologue `0`) |
| `coop_missions` | Mission chapter numbers playable in co-op (`1-29`, `1 2 3`, etc.) |

The engine parses these in [soundrts/campaign.py](../../../soundrts/campaign.py)
...
- `nb_players_min 1` lets a single human start the mission; the engine fills the
  ...
  (`Game._fill_coop_ai_partners` in [soundrts/serverroom.py](../../../soundrts/serverroom.py)).
...
[`tools/generate_raynor_coop_maps.py`](../../../tools/generate_raynor_coop_maps.py) applies co-op layout transforms (wider map, mirrored second player, etc.) into **`N.txt`** for *The Legend of Raynor* only. Other campaigns should author `campaign.txt` + `N.txt` directly.
...
[soundrts/coop_difficulty.py](../../../soundrts/coop_difficulty.py).
