...
> **玩家向说明**：[../player/loadout-cards.md](../player/loadout-cards.md)

战前携带卡可在 **`delay` / `delay_minutes`** 之后再生效，用于「开局预约、稍后增援/发科技」类效果。实现见 `soundrts/cards.py`、`soundrts/card_loadout.py`。

英文版：[../../en/developer/delayed-card-loadout.md](../../en/developer/delayed-card-loadout.md)

相关：[achievement-system.md](achievement-system.md)（卡牌与战前携带总览）、[score-grading-system.md](score-grading-system.md)（结算评分）。
...
- 选卡、扣充能在**进局时**完成；效果在**游戏时间**到达延迟后触发。
...
| 指令 | 说明 |
|------|------|
| `delay <seconds>` | 效果延迟 **秒**（游戏内时间，非真实时间） |
| `delay_minutes <n>` | 等价于 `delay (n×60)` |
| `tech <upgrade_id> [...]` | 到达延迟后授予科技（等同建筑研究完成） |

可与 `spawn`、`resource` **组合**在同一张卡上；**共用同一延迟**，到点一次性执行全部效果。
...
| TTS ID | 中文（ui-zh） | 用途 |
|--------|---------------|------|
| 5387 | 效果将在 | 进局应用延迟卡 / 军械库说明 |
| 5392 | 后生效 | 接在时长之后 |
| 5388 | 携带卡牌效果生效 | 延迟到点触发 |
| 5389 | 在起始位置附近生成 | 军械库：延迟生成单位 |
| 5390 | 额外获得 | 军械库：延迟资源 |
| 5393 | 获得科技 | 军械库：科技效果 |
...
| 卡牌 ID | 效果 | 成就来源 |
|---------|------|----------|
| `card_reinforcements_delayed` | 10 分钟后 3 步兵 | `reinforcement_contract`（5309 援军合约） |
| `card_delayed_melee_weapon` | 8 分钟后 `melee_weapon` | `defeat_expert`（击败专家电脑） |
